...
1. **コールを認証する**
   ...
   このデバイスAPIエンドポイントを使用するには、Adjust APIのトークンが必要です。このトークンをAPIで使用するには、[認証ガイド](#authentication)の設定手順に従ってください。

2. **デバイスの挙動を調査する**

   [inspect device endpoint](#inspect-device)を使って、デバイスと関連するユーザーアクティビティについての情報を返します。

3. **デバイスデータを削除する**

   [forget device endpoint](#forget-device)を使って、Adjustからデバイス情報の履歴を削除します。同一のデバイスで複数のインストールをテストする場合に便利です。
