...
如果您正在使用[Adjust v3 Signature 库](https://help.adjust.com/en/article/sdk-signature)保护应用流量，则可以使用自动化 API 来设置应用的证书指纹。
...
1. 发送一个 POST 请求至`/app`终端，包含以下信息：
   ...
   * `store_id`: 应用在目标 App Store 中的 ID
   * `platform`: 应用程序的目标平台
   * `channel_setup`: 应用合作伙伴模块设置所必需。请参阅[渠道源设置文档](/en/api/app-automation-api/channel-setup/)，了解更多信息。
