...
앱 트래픽 보안을 위해 [Adjust v3 Signature 라이브러리](https://help.adjust.com/en/article/sdk-signature)를 사용하는 경우, 자동화 API를 사용하여 앱의 인증서 핑거프린트를 구성할 수 있습니다.
...
1. 다음 정보와 함께 POST 요청을 `/app` 엔드포인트로 전송합니다.
   ...
   * `platform`: 앱의 타겟 플랫폼
   * `channel_setup`: 앱에 대한 파트너 모듈 설정을 위해 필요 자세한 정보는 [채널 설정 설명](/en/api/app-automation-api/channel-setup/)을 참고하시기 바랍니다. 
