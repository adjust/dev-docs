...
1. **认证您的调用**
   ...
   要使用设备 API 终端，您首先需要获取 Adjust API 识别码。按照此[认证文档](#authentication)中的说明进行操作，了解如何在 API 中应用识别码。

2. **查询设备详情**

   使用[查询设备终端](#inspect-device)来查看设备信息及关联用户活动。

3. **清除设备**

   使用[忘记设备终端](#forget-device)来从 Adjust 清除设备历史。在需要清除设备数据来测试同一设备上的多次安装时，可以使用此功能。
