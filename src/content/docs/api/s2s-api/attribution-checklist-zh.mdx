...
安卓的 Google Play 应用商店广告 ID \(GPS_ADID\) 和 iOS 的广告主 ID \(IDFA\) 均属于[广告 ID](https://help.adjust.com/zh/article/device-identifiers#advertising-ids)。设备用户可以重置或禁止对这两种 ID 的访问。因此，Adjust 还依赖设备 ID 和通用唯一标识符 \(UUID\)（仅限 iOS）进行归因和会话跟踪。如果不重置设备，最终用户则无法重置这两种 ID。
