...
-  **GDPR**: grants the ability to authenticate the GDPR endpoint. The GDPR scope isn't avaible at token creation, but you can set it up in the [Enable S2S authentication](enable-s2s-authentication) section.
...
After adding your necessary scoped tokens, you need to enable S2S authentication for the selected scope endpoints. To enable S2S authentication, follow these steps.
...
6. Switch the **Enable S2S authentication** toggle **ON**. A warning message appears to inform you that only requests sent with configured tokens will work.
7. Customize the authentication scopes by selecting the endpoint or endpoints to authenticate. By default, **Events**, **Sessions**, and **Ad revenue** are selected, but you can select **GDPR** here as well. You must select at least one endpoint.

   ![An authentication scopes modal with scopes selected](@images/s2s/7-select-authentication-scopes.png)

8. Select **Apply**.

You can modify authorization scopes at any time by selecting **Customize scopes**.
