...
---
서버 간\(S2S\) 어트리뷰션과 세션 트래킹을 위해서는 Adjust SDK의 기본 기능을 복제하는 맞춤형 인앱 솔루션이 필요합니다. 서버 간 어트리뷰션 체크리스트는 이 인앱 솔루션의 요구 사항을 정리한 내용입니다. 요구 사항을 충족시키면 Adjust가 수신하는 정보의 보안과 보고의 정확성이 보장됩니다.
...
Adjust는 [Adjust SDK Signature](https://help.adjust.com/en/article/sdk-signature)\(SDK 서명\)를 통해 모든 설치를 보호합니다. 이 서명은 [앱 시크릿](https://help.adjust.com/en/article/sdk-signature#what-is-an-app-secret)을 사용하여 암호화된 해시이며, Adjust SDK에 적용되어 모든 설치 리포트에 포함됩니다. Adjust는 모든 설치 데이터에서 이 해시 정보를 인증하고, 인증되지 않은 어트리뷰션 트래픽을 모두 차단합니다.
...
Android의 Google 플레이 스토어 광고 ID\(GPS\_ADID\) 및 광고주 iOS ID\(IDFA\)는 모두 [광고 ID](https://help.adjust.com/en/article/device-identifiers#advertising-ids)입니다. 디바이스 사용자는 이 두 ID를 쉽게 재설정하거나 액세스를 비활성화 할 수 있습니다. 따라서 Adjust는 어트리뷰션 및 세션 트래킹을 위해 기기 ID 및 범용 고유 식별자\(UUID, iOS 전용\)도 사용하고 있습니다. 최종 사용자는 기기를 재설정해야만 두 ID를 재설정할 수 있습니다.

### [모든 광고 및 기기 ID 수집](gather-every-possible-advertising-and-device-id)

광고 ID는 재설정이 가능합니다. 인앱 보상을 더 받기 위해 광고 ID를 사용자가 고의적으로 재설정하는 경우도 있습니다. 또한, iOS 사용자의 15%는 광고 트래킹 제한\(LAT\)을 설정하여 IDFA에 대한 액세스를 차단하고 있습니다. 이와 같은 이유로 Adjust는 어트리뷰션을 보다 정확하게 하고, 계속해서 인앱 세션을 트래킹할 수 있도록 추가적인 ID를 사용하고 있습니다.
...
사용자가 광고 ID를 재설정하거나 앱을 삭제하고 다시 설치하는 경우, 또는 광고 트래킹 제한\(LAT\)을 활성화하는 경우 Adjust는 해당 IDFA 및 IDFV를 확인할 수 없습니다. 사용자의 인앱 세션을 지속적으로 트래킹하기 위해 Adjust는 디바이스 키 체인에 유지되는 영구적인 로컬 UUID에 의존합니다. Adjust는 이 UUID를 다른 디바이스 정보에 매핑합니다. 이 작업을 통해서 사용자의 인앱 활동을 다음과 같은 경우에도 원활하게 트래킹할 수 있습니다.
...
또한 이 경우 [클릭 인젝션 필터](https://help.adjust.com/en/article/click-injection-filtering)도 작동하지 않습니다. 이로 인해 Android에서 일어나는 주요 모바일 광고 해킹에 취약해집니다. 이 밖에도 Dalvik VM에 대한 명령어 집합 없이는 설치가 가상 기기에서 발생했는지 알 수 없습니다.
...
연동에 필요한 코드는 [Android](/en/sdk/android) 및 [iOS](/en/sdk/ios)용 Adjust SDK 가이드에서 확인하실 수 있습니다. Adjust SDK로 딥링크 리어트리뷰션을 활성화하려면, [Android](/en/sdk/android/features/deep-links) 및 [iOS](/en/sdk/ios/features/deep-links)용 딥링킹 가이드에 명시된 단계를 참조하시기 바랍니다.
