...
正常に計測された全てのイベントは、 `OK`レスポンスを返します。
...
エラーメッセージは、リクエストにどのような問題が発生したかを示します。以下の表からエラーを特定し、設定をご確認いただくか、[support@adjust.com](mailto:support@adjust.com)にお問い合わせください。
...
| `400`        | Ignoring event, earlier unique event tracked                            | ユニークイベントがすでに計測されています。重複するため、このイベントは計測されません。                       |
...
| `403`        | Tracking disabled                                                       | このプラットフォームではトラッキングが有効化されていません。                                                 |
...
| `500`        | Internal error, contact [support@adjust.com](mailto:support@adjust.com) | デバイスデータ更新時に発生した内部エラー。                                                                   |
