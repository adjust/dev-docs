...
---
広告収益を自社で計測する場合は、このデータをサーバー間S2Sで共有することで、Adjustで計測したその他の全ての指標と共に閲覧することができます。これにより、キャンペーンパフォーマンスの全体像が得られ、ユーザー生涯価値を計算し、キャンペーンの広告費用回収率と投資利益率を最適化することができます。
...
[サーバー間（S2S）セキュリティの設定](set-up-s2s-security)
------------------------------------------
...
<tabs> <tab title="成功（Success）" icon="Check"> 

```json
{ "status": "OK" }
```

</Tab> <tab title="エラー（Error）" icon="Cross"> 
