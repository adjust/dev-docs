...
ユーザーが消去リクエストを送信する場合、このAPIエンドポイントに消去リクエストを送信するようにサーバーを構成します。ユーザーの消去リクエストに関する詳細は[消去権に関する記事](https://help.adjust.com/en/article/gdpr)をご覧ください。
<callout type="important"> 

この動作は **恒久的なもの** です。消去リクエストを一旦送信すると、Adjustは、デバイスと以前関連づけられていたデータを復元したり、当該デバイスから再度リクエストを受信したりすることはできません。
</Callout> <codeblock title="エンドポイント" highlight="/gdpr_forget_device"> 
