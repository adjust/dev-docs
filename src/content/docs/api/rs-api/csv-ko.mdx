---
title: "CSV 엔드포인트"
description: "리포트 엔드포인트는 여러 서비스로부터의 데이터를 한 CSV 리포트에 병합할 수 있도록 해줍니다."
...
`GET` 메서드는 리포트 서비스로부터의 필터링된 데이터를 CSV 형식으로 반환합니다. API는 요청에서 전달된 각 파라미터에 대한 데이터 열을 반환합니다. 기본 설정에 따라 파라미터의 슬러그가 해당 열의 헤더로 사용됩니다. 요청에서 `readable_names=true`를 전달하면 읽을 수 있는\(human\-readable\) 이름을 반환할 수 있습니다.
<callout type="important"> 

읽을 수 있는\(human\-readable\) 이름은 업스트림 요청에 따라 달라질 수 있습니다. 일관성을 위해 장기 캠페인에 대한 리포트의 경우 슬러그를 사용하시기 바랍니다. 이벤트 슬러그는 [이벤트 엔드포인트](/en/api/rs-api/events)를 사용하여 조회할 수 있습니다. 
</Callout> 
...
| `reattributed`                 | `String`  | Filter for reattributed users only. Reattribution is when a user who has already installed your app returns to it through a new Adjust\-measureed source.<ul><li>`all` \(기본값\)</li><li>`false`</li><li>`true`</li></ul> | `reattributed=false`                                                                                                                                                                                                                                                                                                                                  |
| `ad_revenue_mode`              | `String`  | 엔드포인트에 의해 반환된 총 광고 매출의 퍼센트. 선택 가능한 옵션은 다음과 같습니다.<ul><li>`GROSS_100`</li><li>`GROSS_85`</li><li>`GROSS_70`</li><li>`GROSS_60`</li></ul>                                                                                     | `ad_revenue_mode=GROSS_70`                                                                                                                                                                                                                                                                                                                            |
