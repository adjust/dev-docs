...
このエンドポイントを使用するには、Adjust APIトークンが必要です。トークンを取得するには、[認証ガイド](/en/api/rs-api/authentication)の手順に従ってください。
...
| `section`                      | `Enum<string id="sl-md0000000">` | クエリ   | 項目をフィルターするセクション。値のリストについては、下の「セクション」の表を参照してください。                                            |
| `[required_filters]__contains` | `String`                         | クエリ   | Enter a search term to look for a full or partial match in the object's id and name fields. |
