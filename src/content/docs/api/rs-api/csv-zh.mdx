...
人类可读名称可能基于上游要求发生变化。对于长期持续报告，请使用 slug 确保报告前后一致。您可以使用[事件终端](/en/api/rs-api/events)来获取事件 slug。
</Callout> 
...
| `reattributed`                 | `String`  | Filter for reattributed users only. Reattribution is when a user who has already installed your app returns to it through a new Adjust\-measureed source.<ul><li>`all` \(默认\)</li><li>`false`</li><li>`true`</li></ul> | `reattributed=false`                                                                                                                                                                                                                                                                                                                            |
