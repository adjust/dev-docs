...
当您选择平台时，针对 [CTV 监测](https://help.adjust.com/en/article/ctv-advision)以及 [PC 和主机监测](https://help.adjust.com/en/article/measurement-for-pc-and-console-games)的 S2S 会话监测会自动启用。

### 设置 S2S 安全\{\#set\-up\-s2s\-security\}

[实施 S2S 安全机制](/en/api/s2s-api/security)，保护您的 S2S 活动不受侵害，防止伪造请求。在 Adjust 控制面板中生成识别码，并在每个传入请求中加入该识别码。
...
使用[原始数据导出](https://help.adjust.com/en/article/raw-data-exports)时，您可以在所有 S2S 请求中加入 “统一回传参数”，以便在原始数据中添加自定义参数。这种方法通常用来在原始数据导出中加入您的内部用户 ID。
...
* 设备首次会话会被记录为 “安装” 活动。
* 后续会话会被记录为 “会话” 活动。
* 如果满足[再归因条件](https://help.adjust.com/en/article/reattribution)，则会被记录 “再归因” 或 “再归因重装” 活动。
...
这是 Adjust 成功记录设备首次会话时的响应格式。如有需要，您可以使用 Adjust [测试控制台](https://help.adjust.com/en/article/testing-console)遗忘设备，并进行多次测试。
<codeblock title="响应"> 
...
成功发送至少 1 次设备会话请求后，您就可以发送[安装后事件](/en/api/s2s-api/events)了。这些事件通常代表营销目标，广告渠道可使用这些信息来优化推广活动。
<tabs> <tab title="Swift" sync="swift"> 
...
使用[原始数据导出](https://help.adjust.com/en/article/raw-data-exports)时，您可以在特定事件请求中加入自定义 “回传参数”，以添加事件层级自定义数据。例如，如果应用中发生购买事件，您就可以在原始数据中加入该事件的内部交易 ID。
