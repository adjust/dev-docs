...
如果您想在商业分析中使用外部设备 ID，则可以将其作为[会话回传参数](/en/sdk/cordova/features/session-parameters)发送。
...
您可以在用户的[归因信息](/en/sdk/cordova/features/attribution)中访问成本数据。
...
默认情况下，Adjust SDK 会在应用打开时启动。如果您想通过[会话参数](/en/sdk/cordova/features/session-parameters)发送的数据在应用打开时不可用，那么则可以延迟 SDK 启动。当您想发送唯一标识符等信息时，这一方法尤其有效。
