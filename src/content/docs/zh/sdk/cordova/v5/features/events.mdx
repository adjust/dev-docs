...
versions: 
    ...
    - 
        ...
        value: "v4"
redirects: 
    v4: "/en/sdk/cordova/v4/features/events"
---
...
</CodeBlock> <codeblock title="事件日志" highlight="{range: 22-23}" collapse="7-21, 24-46"> 
...
</CodeBlock> <codeblock title="事件日志" highlight="{range: 7-8}"> 
...
在此示例中，每次用户与某个按钮互动时，我们就监测到一个带有 *`g3mfiw`* 标签的事件。该函数会使用 `setDeduplicationId` 方法将 `deduplicationId` 设为 *`5e85484b-1ebc-4141-aab7-25b869e54c49`* 。
...
</CodeBlock> <codeblock title="事件日志" highlight="{range: 7}"> 
...
</CodeBlock> <codeblock title="index.js" highlight="{range: 31-32}"> 
...
</CodeBlock> <codeblock title="事件日志" highlight="{range: 4}"> 
