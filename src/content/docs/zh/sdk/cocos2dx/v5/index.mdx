...
1. 将 Install Referrer 库添加至您的`build.gradle`文件。
...
## 6. 集成 Adjust SDK{#integrate-the-adjust-sdk}
...
1. 在文件顶部包含`Adjust/Adjust2dx.h`类。

1. 使用以下参数实例化`AdjustConfig2dx`对象：
...
1. 应用启动后，尽快调用`Adjust2dx::initSdk`方法并将`AdjustConfig2dx`实例作为参数传递。
