...
| 值              | 数据类型 | 描述                                                                          |
| --------------- | -------- | ----------------------------------------------------------------------------- |
| `adid`          | 字符串   | 设备的唯一 Adjust 标识符。                                                    |
| `tracker_token` | 字符串   | 与归因关联的推广活动识别码。                                                  |
| `tracker_name`  | 字符串   | 推广活动链接名称                                                              |
| `network`       | 字符串   | 与推广活动关联的渠道。                                                        |
| `campaign`      | 字符串   | 与归因关联的推广活动名称。                                                    |
| `adgroup`       | 字符串   | 与归因关联的广告组。                                                          |
| `creative`      | 字符串   | 与归因关联的素材。                                                            |
| `click_label`   | 字符串   | 与归因关联的 [click label](https://help.adjust.com/zh/article/user-rewards)。 |
| `state`         | 字符串   | 归因当前状态。`installed` 或 `reattributed`                                   |
...
用户安装您的应用时，Adjust 会将该安装归因至一个推广活动。Adjust SDK 可提供赢得安装归因的推广活动细节。要返回次信息，请调用 `getAttribution` 方法。
...
要设置 referrer，请调用 `setReferrer` 方法并以 URL 编码 **字符串** 参数的形式传递 referrer。
