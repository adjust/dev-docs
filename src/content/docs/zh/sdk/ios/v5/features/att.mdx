...
</CodeBlock> </Tab> <tab title="Objective-C" sync="objc"> <codeblock highlight="{range: 4}"> 
...
禁用 App Tracking Transparency \{\#disable\-att\-framework\}
---------------------------------------------------------------
...
若要禁用 Adjust SDK 与 `AppTrackingTransparency.framework` 的自动交互，请在 SDK 初始化之前调用 `ADJConfig` 实例上的 `disableAppTrackingTransparencyUsage` 方法。
<callout type="note"> 

即便采用该设置，SDK 依旧会允许您使用 `requestAppTrackingAuthorizationWithCompletionHandler` 和 `appTrackingAuthorizationStatus` 方法。
</Callout> <tabs> <tab title="Swift" sync="swift"> <codeblock highlight="{range: 6}"> 
...
</CodeBlock> </Tab> <tab title="Objective-C" sync="objc"> <codeblock highlight="{range: 5}"> 
