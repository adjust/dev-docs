...
slug: zh/sdk/react-native/v4/features/skad
versions:
   - label: v5
     value: v5
     default: true
   - label: v4
     value: v4
redirects:
   v5: /zh/sdk/react-native/features/skad
---
...
| `lockWindow`  | `NSNumber` | 是否在转化窗口结束前发送回调。`1` 表示回调会在转化窗口结束前发送。在 SKAdNetwork 4\.0 回调中默认为 `0`；在更旧的 SKAdNetwork 版本中默认为 `nil`。                                                              |
