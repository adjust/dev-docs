...
* `isEnabled` \(`boolean` 또는 `null`\): 서드파티 공유의 활성화 여부. `true` 전달로 서드파티 공유를 활성화하거나 `false` 전달로 서드파티 공유를 비활성화합니다. 현재 `isEnabled` 토글의 상태에 대한 파트너 공유 설정을 조정하려면 `null`을 전달합니다.
...
* `urlStrategyDomains` \(`string[]`\): SDK 트래픽을 전송할 엔드포인트의 배열.
