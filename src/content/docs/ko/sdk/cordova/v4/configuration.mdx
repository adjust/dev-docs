...
slug: "en/sdk/cordova/v4/configuration"
sidebar-position: 1
versions: 
    - 
        label: "v5"
    ...
    - 
        ...
        value: "v4"
redirects: 
    v5: "/en/sdk/cordova/configuration"
---
...
config 객체를 인스턴스화 하려면 새로운 `AdjustConfig` 인스턴스를 생성하고 다음 파라미터를 전달합니다.

* `appToken` `string`: [Adjust 앱 토큰](https://help.adjust.com/en/article/app-token-and-reporting-currency#view-your-app-details)입니다.
* `environment` \(`AdjustEnvironment`\): SDK를 실행할 환경입니다. 테스트를 위해 샌드박스 모드에서 SDK를 실행하려면 `AdjustConfig.EnvironmentSandbox`를 전달합니다. 릴리즈를 위해 프로덕션 모드에서 SDK를 실행하려면 `AdjustConfig.EnvironmentProduction`을 전달합니다.
...
읽기 전용 구성 \{\#readonly\-configuration\}
------------------------------------------

**읽기 전용** 구성 옵션은 SDK를 초기화하기 **전** 에 `AdjustConfig` 인스턴스에 설정되어 있습니다. SDK 실행 중에는 이 옵션을 변경할 수 없습니다. 사용할 옵션은 `Adjust.create()`를 실행하기 전에 설정해야 합니다.
...
다음 인수로 `setLogLevel` 메서드를 호출하여 로그 수준을 설정할 수 있습니다.
...
외부 기기 ID를 기기에 대한 커스텀 ID로서 사용할 수도 있습니다. 이는 기타 시스템과 통일성을 유지하고 싶은 경우 유용합니다. 다음 인수로 `setExternalDeviceId` 메서드를 호출하여 외부 기기 ID를 할당합니다.
...
비즈니스 애널리틱스에서 외부 디바이스 ID를 사용하려면 이를 [세션 콜백 파라미터](/en/sdk/cordova/features/session-parameters)로 전송해야 합니다.
...
기본 설정에 따라 Adjust SDK는 사용자의 어트리뷰션에서 비용 데이터를 전송하지 않습니다. 비용 데이터 전송을 활성화하여 이 데이터를 전송하도록 SDK를 구성할 수 있습니다. 비용 데이터 전송을 활성화하려면 다음 인수로 `setNeedsCost` 메서드를 호출합니다.

* `needsCost` \(`boolean`\): 비용 데이터 전송을 활성화하거나 `true`, 비용 데이터 전송을 비활성화하려면 `false`로 설정합니다.

비용 데이터는 사용자의 [어트리뷰션 정보](/en/sdk/cordova/features/attribution)에서 액세스할 수 있습니다.
...
기본값에 따라 Adjust SDK는 앱이 백그라운드에서 실행 중일 때 요청 전송을 일시 중지합니다. 백그라운드 리코딩을 활성화하여 백그라운드에서 요청을 보내도록 SDK를 구성할 수 있습니다. 백그라운드 리코딩을 활성화하려면 다음 인수와 함께 `setSendInBackground` 메서드를 호출합니다.
...
config 객체에는 해당 동작을 제어하는 boolean `eventBufferingEnabled` 속성이 포함되어 있습니다. 이벤트 버퍼링을 활성화하려면 다음 인수로 `setEventBufferingEnabled` 메서드를 호출합니다.
...
기본 설정에 따라 Adjust SDK는 앱이 실행되자마자 시작됩니다. 실행 시 이용할 수 없는 데이터를 [세션 파라미터](/en/sdk/cordova/features/session-parameters) 로 전송하려면 SDK의 시작을 지연시킬 수 있습니다. 이는 고유 ID와 같은 정보를 전송할 때 유용합니다.
...
* `isOffline` \(`boolean`\): 오프라인 모드를 활성화하거나 `true`, 오프라인 모드를 비활성화하려면 `false`로 설정합니다.
...
config 객체에는 푸시 토큰을 저장할 수 있는 문자열 `pushToken` 속성이 포함되어 있습니다. `setPushToken` 메서드를 호출하고 다음 인수를 전달하여 언제든지 이 속성을 업데이트할 수 있습니다.
...
* `isEnabled` \(`boolean`\): SDK를 활성화하거나 `true`, SDK를 비활성화하려면 `false`로 설정합니다.
...
```js
isEnabled(): Promise<boolean id="sl-md0000000">
```
