...
* `domains` \(`Array<string id="sl-md0000000">`\): SDK 트래픽을 전송할 데이터 레지던스 국가 또는 엔드포인트. 기본 설정에 따라 Adjust SDK는 모든 데이터를 Adjust의 엔드포인트로 전송합니다. 커스텀 엔드포인트를 설정하려면 `urlStrategy`의 `domains` 속성에 엔드포인트를 전달합니다.
