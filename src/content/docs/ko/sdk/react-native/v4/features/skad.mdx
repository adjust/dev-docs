...
slug: "en/sdk/react-native/v4/features/skad"
versions: 
    - 
        label: "v5"
        value: "v5"
    ...
    - 
        ...
        value: "v4"
redirects: 
    v5: "/en/sdk/react-native/features/skad"
---
...
SKAdNetwork 4\.0 포스트백은 광고주들에게 사용자 관련 인사이트를 더 많이 제공하기 위해 추가적인 정보를 포함하고 있습니다. Adjust 서버가 전환값을 업데이트할 때 이 추가 정보가 페이로드에 전송됩니다. `setSkad4ConversionValueUpdatedCallbackListener` 콜백 메서드로 이 정보에 액세스하실 수 있습니다.
</MinorVersion> 
...
| `lockWindow`       | `NSNumber` | 전환 윈도우가 종료되기 전에 포스트백을 전송할지 여부. `1`은 전환값 윈도우 종료 전에 포스트백이 전송됨을 의미합니다.<br/>SKAdNetwork 4\.0 포스트백에서는 `0`, 그 이전 SKAdNetwork 버전에서는 `nil`로 기본 설정되어 있습니다.                                                                   |
