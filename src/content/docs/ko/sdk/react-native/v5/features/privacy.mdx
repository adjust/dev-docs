...
* Adjust 내부 시스템과 데이터베이스에서 해당 사용자의 모든 개인정보 기록을 영구적으로 삭제합니다.
...
Adjust는 기본 설정에 따라 앱 설정에서 구성한 모든 파트너와 모든 지표를 공유합니다. Adjust SDK를 사용하여 파트너별로 서드파티 공유 설정을 업데이트할 수 있습니다. 이를 실행하려면 `addPartnerSharingSetting` 메서드를 다음의 인수와 함께 호출해야 합니다.
...
특정 파트너에 대해 특정 지표에 `false` 값을 설정하면 Adjust는 해당 파트너와의 해당 지표 공유를 중단합니다.
<callout type="tip"> 
...
앱을 어린이 앱으로 표시하려면 `enablePlayStoreKidsCompliance` 메서드를 호출합니다.
