...
Adjust SDK는 기본 설정에 따라 사용자의 어트리뷰션에서 비용 데이터를 전송하지 않습니다. 비용 데이터 전송을 활성화하면 비용 데이터를 전송하도록 SDK를 구성할 수 있습니다. `enableCostDataInAttribution` 메서드를 config 인스턴스에서 호출하면 비용 데이터 전송을 활성화할 수 있습니다.
...
Adjust SDK는 기본 설정에 따라 앱이 백그라운드에서 실행 중일 때 요청 전송을 일시 중지합니다. 백그라운드 리코딩을 활성화하면 백그라운드에서 요청을 보내도록 SDK를 구성할 수 있습니다. `enableSendingInBackground` 메서드를 config 인스턴스에서 호출하면 백그라운드 기록을 활성화할 수 있습니다.
