...
1. `AndroidManifest.xml` 파일을 열고 `<application>` 요소를 찾습니다.

1. `<application>` 요소에 `android:name` 어트리뷰션을 추가하고 애플리케이션 클래스의 이름으로 설정합니다. 예를 들어 `Application` 클래스 이름이 `GlobalApplication`인 경우 다음과 같이 설정합니다.
