...
[Android 앱 링크](https://help.adjust.com/ko/article/set-up-android-app-links)의 경우, `AndroidManifest.xml` 파일에 `intent` 필터를 추가하여 앱이 처리할 수 있는 URL을 지정합니다. `intent` 필터에 `android:autoVerify="true"` 어트리뷰션과 함께 `data` 요소를 포함합니다.
