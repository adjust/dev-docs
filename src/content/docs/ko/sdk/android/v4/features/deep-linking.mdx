...
slug: "en/sdk/android/v4/features/deep-links"
versions: 
    - 
        label: "v5"
        value: "v5"
    ...
    - 
        ...
        value: "v4"
redirects: 
    v5: "/ko/sdk/android/features/deep-links"
---
...
[딥링크에 대해 Android 앱 링크를 설정](#configure-android-app-links)하는 경우에도 스킴 이름을 구성해야 합니다. 이는 앱 스킴\(`appName://`\)이 푸시 알림이나 YouTube, X와 같은 특정 환경에서 자동 라우팅에 사용되기 때문에 필요합니다. Android 앱 링크가 특정 환경에서 앱을 열 수 없는 경우 앱 스킴은 라우팅 로직의 폴백 역할을 합니다.
</Callout> 
...
딥링크를 위해 Android 앱 링크를 설정하는 경우에도 [스킴 이름을 구성](#configure-your-scheme-name)해야 합니다.

[Android 앱 링크](https://help.adjust.com/en/article/set-up-android-app-links)의 경우, `AndroidManifest.xml` 파일에 `intent` 필터를 추가하여 앱이 처리할 수 있는 URL을 지정합니다.  `intent` 필터에 `android:autoVerify="true"` 어트리뷰션과 함께 `data` 요소를 포함합니다.
...
Adjust를 통해 딥링크를 사용하여 리인게이지먼트 캠페인을 집행할 수 있습니다. 자세한 내용은 [Campaign Lab에서 딥링크를 설정하는 방법](https://help.adjust.com/en/article/deep-links)을 확인하시기 바랍니다.
