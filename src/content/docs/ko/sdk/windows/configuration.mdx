...
앱이 기기에 사전 설치된 경우 디폴트 링크 토큰을 설정할 수 있습니다. 사용자가 사전 설치된 앱을 처음으로 실행할 때 해당 설치가 디폴트 링크 토큰으로 어트리뷰션됩니다. 디폴트 링크 토큰을 설정하려면 [Adjust 링크 토큰](https://help.adjust.com/en/article/links#adjust-link-token)을 config 인스턴스의 `DefaultTracker` 속성에 할당합니다.
