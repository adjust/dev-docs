...
versions:
   ...
   - label: v4
     value: v4
redirects:
   v4: /ko/sdk/unity/v4/features/preinstalled
---

Adjust SDK를 사용하여 사용자의 기기에 사전 설치된 앱의 활동을 기록할 수 있습니다. 이 방법으로 캠페인을 통해 앱을 다운로드하지 않은 사용자의 정보를 전송할 수 있습니다.

`AdjustConfig` 인스턴스의 `IsPreinstallTrackingEnabled` 속성을 `bool` 값으로 설정하여 사전 설치 측정의 활성화 여부를 관리합니다.
...
Adjust SDK가 사전 설치 정보에 액세스할 수 있도록 이 정보가 저장된 파일 경로를 `PreinstallFilePath` 메서드로 전달합니다.
...
2. 이 토큰을 복사한 뒤 앱 델리게이트 파일에 있는 `AdjustConfig` 인스턴스의 `DefaultTracker` 속성에 할당합니다.
