...
Adjust SDK가 사전 설치 정보에 액세스할 수 있도록 이 정보가 저장된 파일 경로를 `AdjustConfig` 인스턴스의 `PreinstallFilePath` 속성에 할당합니다.
...
2. 이 토큰을 복사하고 앱 델리게이트 파일에 있는 `AdjustConfig` 인스턴스의 `DefaultTracker` 속성에 할당합니다.
