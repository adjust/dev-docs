...
S2SイベントをiOS 14以降のアプリに設定している場合、Adjustはconversion valueをS2Sレスポンスとしてサーバーに送信します。その後、お客様のサーバー側でアプリ内のconversion valueを更新する必要があります。この仕組みの詳細については、[iOS 14以降でのサーバー間S2Sイベントのサポート](https://help.adjust.com/en/article/support-for-s2s-events)を参照してください。
<callout type="note"> 
