...
これらのソリューションは[パートナーパラメーター](/en/sdk/android/features/events#add-partner-parameters)をサポートしません。カスタムパラメーターを使用してネットワークにイベントを送信する必要がある場合は（例：動的リターゲティングのサポート）、これらのイベントをSegment SDKとAdjust SDKの両方で設定してください。SegmentダッシュボードでこれらのイベントをAdjustにリンクしないでください。
</Callout> 
