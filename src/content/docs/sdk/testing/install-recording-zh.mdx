...
5. 如果您此前曾使用此设备进行测试：
   ...
   1. 从测试设备中删除您的应用。
   2. 在测试控制台中清除设备的广告 ID。请按照[AppView 中的测试控制台](https://help.adjust.com/en/article/testing-console-suite#forget-device)中的说明进行操作。
   3. 或者调用[遗忘设备终端](/en/api/device-api/forget/)

6. [创建测试链接](https://help.adjust.com/en/article/links)。

   1. 如果要测试概率模型，则需要在 **应用层级** 或链接层级启用[概率模型](https://help.adjust.com/en/article/attribution-settings-app-level)
   2. 如果要测试[设备 ID 匹配](https://help.adjust.com/en/article/attribution-methods#deterministic-attribution)，则需要将设备广告 ID 添加至您的推广活动 URL：
