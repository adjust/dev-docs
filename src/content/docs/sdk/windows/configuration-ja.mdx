...
* `logDelegate` （ **Action** ）：ログが委譲される関数。[`setLogDelegate`メソッド](#configure-a-log-delegate)を使用しても設定できます。
...
* `offline` （Boolean）：オフラインモードを有効化するには`true`に、オフラインモードを無効化するには`false`に設定します。
