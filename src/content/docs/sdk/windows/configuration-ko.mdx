...
앱이 기기에 사전 설치된 경우 디폴트 링크 토큰을 설정할 수 있습니다. 사용자가 사전 설치된 앱을 처음으로 실행할 때 해당 설치가 디폴트 링크 토큰으로 어트리뷰션됩니다. 디폴트 링크 토큰을 설정하려면 [Adjust 링크 토큰](https://help.adjust.com/ko/article/links#adjust-link-token)을 config 인스턴스의 https://help.adjust.com/ko/article/links\#adjust\-link\-token 속성에 할당합니다.
...
기본 설정에 따라 Adjust SDK는 앱이 실행되자마자 시작됩니다. 시작할 때 사용할 수 없는 데이터를 [세션 파라미터](/ko/sdk/flutter/features/session-parameters)로 전송하려면 SDK의 시작을 지연시킬 수 있습니다. 이는 고유 식별자와 같은 정보를 전송할 때 유용합니다.
