...
![Package.appxmanifest 配置的屏幕截图。](https://images.ctfassets.net/5s247im0esyq/6LF0U8E3MD8HS6sBfDMgUc/627613961e6a9bbb7d5a76372ca29e98/deep-link-setup-windows-sdk.png)
...
如果用户与链接互动，而该链接 `deep_link` 参数中包含您的 **唯一 Scheme 名称** ，那么您的应用就会打开。参数该值必须在 URL 中进行 URL 编码。Adjust SDK 将以未编码的文本形式传送结果。
...
您可以配置 Adjust SDK，使其在接收到延迟深度链接时调用一个委托函数。委托函数会以 **字符串** 参数的形式接收深度链接。SDK 从 Adjust 服务器接收到深度链接内容后，就会通过该委托函数传送，并预计收到 **布尔式** 返回值。该值代表 SDK 是否应启动 **OnActivated** 事件处理程序。
...
Adjust 支持使用深度链接进行再交互推广活动。要了解更多信息，请参阅[如何在深度链接中附加归因数据](https://help.adjust.com/en/article/manage-reattributions#track-reattributions-via-deeplink-clicks)。
</Callout> 
...
如果用户点击了 Adjust 深度链接，需要被再归因，那么[归因回传](/en/sdk/windows/features/attribution)就会被触发。
