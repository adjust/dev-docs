...
![Package.appxmanifest 스크린샷 구성.](https://images.ctfassets.net/5s247im0esyq/6LF0U8E3MD8HS6sBfDMgUc/627613961e6a9bbb7d5a76372ca29e98/deep-link-setup-windows-sdk.png)
...
디퍼드 딥링크 수신 시 델리게이트 함수를 호출하도록 Adjust SDK를 구성할 수 있습니다. 이 델리게이트 함수는 딥링크를 **문자열\(string\)** 인수로 수신합니다. SDK에서 Adjust 서버의 딥링크 컨텐츠를 수신하면 델리게이트 함수를 통해 전달하며 **boolean** 반환값을 예상합니다. 이 값은 SDK가 **OnActivated** 이벤트 처리기를 출시할지 여부를 결정합니다.
...
Adjust를 통해 딥링크를 사용하여 리인게이지먼트 캠페인을 집행할 수 있습니다. 자세한 정보는 Adjust의 [어트리뷰션 데이터를 딥링크에 추가하는 방법](https://help.adjust.com/en/article/manage-reattributions#track-reattributions-via-deeplink-clicks)을 참조하시기 바랍니다.
</Callout> 
...
Adjust 딥링크 클릭으로 사용자가 리어트리뷰션되어야 하는 경우 이는 [어트리뷰션 콜백](/en/sdk/windows/features/attribution)을 트리거합니다.
