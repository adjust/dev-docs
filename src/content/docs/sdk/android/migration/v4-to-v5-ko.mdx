...
SDK v4에서는 [COPPA 준수](/ko/sdk/android/features/privacy#coppa-compliance)가 `AdjustConfig` 클래스를 사용하여 설정되며 SDK 초기화 중에 읽히게 됩니다. 따라서 SDK가 초기화된 후에는 해당 값을 업데이트할 수 없습니다.
...
SDK v4에서는 `AdjustConfig` 클래스를 사용하여 앱을 [플레이 스토어 어린이용 앱](/ko/sdk/android/features/privacy#play-store-kids-apps)으로 표시할 수 있습니다. 이 속성은 SDK 초기화 중에 읽히기 때문에 SDK가 초기화된 후에는 해당 값을 업데이트할 수 없습니다.
