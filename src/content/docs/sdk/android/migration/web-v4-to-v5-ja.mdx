...
SDK v4では、 [COPPAコンプライアンス](/en/sdk/android/features/privacy#coppa-compliance)は、ブーリアン引数でconfigオブジェクトの`setCoppaCompliantEnabled`メソッドを呼び出すことで設定されます。
</MinorVersion> 
...
SDK v4では、ブーリアン引数でconfigオブジェクトの`setPlayStoreKidsAppEnabled`メソッドを呼び出すことで、[アプリをPlayストアの子供向け](/en/sdk/android/features/privacy#play-store-kids-apps)アプリとしてマークできます。
</MinorVersion> 
