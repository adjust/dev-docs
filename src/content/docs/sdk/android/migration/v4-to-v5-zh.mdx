...
### [签名设置](signature-setup)

SDK v5 包含[SDK 签名库](https://help.adjust.com/en/article/sdk-signature)。您可以按照[签名库集成指南](/en/sdk/android/integrations/signature-library)中的测试步骤对此进行测试。
...
在 SDK v4 中，您可以通过布尔参数在`AdjustConfig`实例上调用`setPlayStoreKidsAppEnabled`方法将应用标记为[Play 商店儿童应用](/en/sdk/android/features/privacy#play-store-kids-apps)。
</MinorVersion> <tabs> <tab title="Kotlin" sync="kotlin"> 
