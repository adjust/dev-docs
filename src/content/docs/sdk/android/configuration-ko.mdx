...
* `context` \(컨텍스트\): 앱이 실행되는 [Android 컨텍스트](https://developer.android.com/reference/android/content/Context). 앱의 현재 컨텍스트를 가져오려면 `this`를 전달합니다.
* `appToken` \(문자열\): [Adjust 앱 토큰](https://help.adjust.com/en/article/app-token-and-reporting-currency#view-your-app-details)입니다.
* `environment` \(문자열\): SDK를 실행할 환경. 테스트를 위해 샌드박스 모드에서 SDK를 실행하려면 `AdjustConfig.ENVIRONMENT_SANDBOX`를 전달합니다. 릴리즈를 위해 프로덕션 모드에서 SDK를 실행하려면 `AdjustConfig.ENVIRONMENT_PRODUCTION`를 전달합니다.
...
**읽기 전용** 구성 옵션은 SDK를 초기화하기 **전** 에 `AdjustConfig` 인스턴스에 설정되어 있습니다. SDK 실행 중에는 이 옵션을 변경할 수 없습니다. 사용할 옵션은 `Adjust.onCreate()`를 실행하기 전에 설정해야 합니다.
...
기본 설정에 따라 Adjust SDK는 앱이 백그라운드에서 실행 중일 때 요청 전송을 일시 중지합니다. 백그라운드 기록을 활성화하여 백그라운드에서 요청을 보내도록 SDK를 구성할 수 있습니다. 백그라운드 기록을 활성화하려면 다음 파라미터와 함께 config 인스턴스에서 `setSendInBackground` 메서드를 호출합니다.

* `sendInBackground` \(boolean\): 백그라운드 전송을 활성화하려면 `true`, 백그라운드 전송을 비활성화하려면 `false`로 설정합니다.
...
* `eventBufferingEnabled` \(boolean\): 이벤트 버퍼링을 활성화하려면 `true`, 이벤트 버퍼링을 비활성화하려면 `false`로 설정합니다.
...
기본 설정에 따라 Adjust SDK는 앱이 실행되자마자 시작됩니다. 시작할 때 사용할 수 없는 데이터를 [세션 파라미터](/en/sdk/android/features/session-parameters)로 전송하려면 SDK의 시작을 지연시킬 수 있습니다. 이는 고유 식별자와 같은 정보를 전송할 때 유용합니다.
...
* `enabled` \(boolean\): 오프라인 모드를 활성화하려면 `true`, 오프라인 모드를 비활성화하려면 `false`로 설정합니다.
...
* `token` \(문자열\): 푸시 토큰.
* `context` \(컨텍스트\): 앱이 실행되는 [Android 컨텍스트](https://developer.android.com/reference/android/content/Context). 앱의 현재 컨텍스트를 가져오려면 `getApplicationContext()`를 전달합니다.
...
* `enabled` \(boolean\): SDK를 활성화하려면 `true`, SDK를 비활성화하려면 `false`로 설정합니다.
