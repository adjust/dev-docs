...
```groovy
dependencies {
   implementation 'com.adjust.sdk:adjust-android:v$ANDROID_VERSION'
   implementation 'com.android.installreferrer:installreferrer:2.2'
   // Add the following if you're using the Adjust SDK inside web views on your app
   implementation 'com.adjust.sdk:adjust-android-webbridge:v$ANDROID_VERSION'
}
```
...
如果您的应用并非面向 Google Play 商店，那么请添加下列权限，以读取设备的互联网连接状态：

```xml
<uses-permission android:name="android.permission.ACCESS_WIFI_STATE" id="sl-md0000000"/>
```

Adjust SDK 默认包含 `com.google.android.gms.AD_ID` 权限。如果您要让应用符合 COPPA \(儿童在线隐私保护法\) 规定，或者应用并非面向 Google Play 商店，请务必使用 `remove` 命令移除权限。
...
借助 Samsung referrer 插件，您可以记录面向小米设备应用的 Install Referrer 值。请查看插件相关文档，了解安装信息。
