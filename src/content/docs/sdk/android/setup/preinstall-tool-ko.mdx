...
---
앱이 기기에 사전 설치된 경우에는 사전 설치 트래커를 사용하여 사용자를 어트리뷰션해야 합니다. Adjust의 스토어 & 사전 설치 툴\(`adjust-dtt`\)은 APK에 이러한 정보를 넣을 수 있도록 해줍니다. Adjust SDK는 `adjust-dtt`에 의해 삽입된 정보를 읽습니다. 이를 통해 사전 설치된 앱의 사용자를 특정 Adjust 트래커로 어트리뷰션할 수 있습니다.
...
[사전 설치 트래커 정보 추가](add-preinstall-tracker-information)
-----------------------------------------------------
...
### [2단계: 트래커 토큰 생성하기](step-2-create-tracker-token)

다음은 Adjust 대시보드에서 캠페인 링크를 생성해야 합니다. 링크 토큰은 사전 설치 앱에 대한 기본 설정 캠페인으로 사용하게 됩니다. 따라서 사전 설치된 앱이 있는 모든 사용자는 해당 토큰에 어트리뷰션 됩니다.

### [3단계: config 파일 생성하기](step-3-create-a-config-file)

keystore 파일과 캠페인 토큰이 모두 준비되면 config 파일을 생성할 수 있습니다. `adjust-dtt`는 설정을 읽기 위해 `adjust-config.yaml` config 파일을 사용합니다. 이 파일은 설정해야 하는 모든 앱 스토어에 대한 설정을 포함합니다.
<table> 
...
| `default_tracker` | `String` | Per\-store | 사전 설치를 어트리뷰션할 Adjust 캠페인 토큰 |
