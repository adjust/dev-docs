...
您需要拥有默认密钥库文件或自定义密钥库文件访问权限，才能使用`adjust-dtt`，因为使用`apktool`解包 APK 时，所有签名都会被 apktool 移除。重新打包 APK 并不会重新启用签名。您需要使用签名信息运行`jarsigner`，才能为 APK 重新签名，然后向 Android 设备发送。

### 步骤 2\. 创建跟踪码

下一步，请在 Adjust 控制面板中创建推广活动链接。该链接识别码将会是您预安装应用的默认推广链接。也就是说，所有预先安装应用的用户都会被归因至该识别码。

### 步骤 3\. 创建 config 文件

具备密钥库文件和推广识别码后，您就可以创建配置 \(config\) 文件了。`adjust-dtt`使用一个`adjust-config.yaml`配置文件来读取您的设置。该文件中包含需要您设置的不同应用商店选项。
<table> 

|        参数         | 数据类型 | 全局设置或按商店设置 |          描述           |
|-------------------|------|------------|-----------------------|
| `apk_path`        | 字符串  | 两者兼需       | APK 的绝对路径。            |
| `keystore_path`   | 字符串  | 两者兼需       | 密钥库文件的绝对路径。           |
| `keystore_pass`   | 字符串  | 两者兼需       | 密钥库签名密码。              |
| `keystore_alias`  | 字符串  | 两者兼需       | 密钥库名称。                |
| `default_tracker` | 字符串  | 按商店设置      | 用来归因预安装的 Adjust 推广识别码 |
