---
title: "多进程应用"
description: "为多进程应用设置 Android Adjust SDK"
slug: "en/sdk/android/setup/multi-process"
multiVersion: true
---
<sdkversion version="v5"> <callout type="important"> 

如果在未设置进程名称的情况下，于多个进程中调用 SDK，就会初始化多个 SDK 实例，因为 Android 中不同进程的内存空间也不同。请务必确保设置主进程名称， **或** 仅在单个进程中使用 Adjust SDK。
</Callout> 
...
```xml
<activity     android:name=".YourActivity"     android:process=":YourProcessName" id="sl-md0000000">
</activity>

<service     android:name=".YourService"     android:process=":YourProcessName" id="sl-md0000000">
</service>
```
...
Adjust SDK 当前不支持应用中一个进程以上的监测。如果您的应用采用多进程，那么可以在 [`AdjustConfig` 对象](/en/sdk/android/configuration)中设置主进程名称。
...
```xml
<application   android:name=".YourApp"   android:icon="@drawable/ic_launcher"   android:label="@string/app_name"   android:theme="@style/AppTheme"   android:process=":YourMainProcessName" id="sl-md0000000">
</application>
```
...
</SdkVersion> <sdkversion version="v4"> <callout type="important"> 
...
</CodeBlock> <codeblock title="AndroidManifest.xml"> 
