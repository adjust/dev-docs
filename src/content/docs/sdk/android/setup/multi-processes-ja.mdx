...
プロセス名を設定せずに複数のプロセスでSDKを呼び出すと、SDKの複数のインスタンスが初期化されます。これは、Androidではプロセスがメモリースペースを共有しないためです。必ずメインプロセス名を設定するか、 **あるいは** Adjust SDKを1つのプロセスのみで使用してください。
</Callout> 
...
現在Adjust SDKは、1つのアプリで複数のプロセスの計測をサポートしていません。アプリがマルチプロセスを使用している場合、[`AdjustConfig`オブジェクト](/en/sdk/android/configuration)でメインのプロセス名を設定してください。
