...
Original Equipment Manufacturer (OEM) partners can leverage Android system properties to attribute preinstalled apps. The <Abbr>OEM (Original Equipment Manufacturer)</Abbr> writes attribution information to a file and adds its path to the system properties. The Adjust SDK reads this file on initialization to attribute the install.
