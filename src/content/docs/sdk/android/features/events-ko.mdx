...
* 새 Adjust 이벤트 인스턴스를 만들고 이벤트 토큰을 `String` 인수로 전달합니다.
...
</Tab> </Tabs> <codeblock title="이벤트 로그" collapse="6-46"> 
...
[구매 검증](https://help.adjust.com/en/article/purchase-verification)을 활성화한 경우 검증을 위해 구매 이벤트와 함께 추가 정보를 전송해야 합니다.Adjust의 서버가 이벤트 객체를 통해 해당 정보를 수신하면, 해당 정보를 Google에 전달하여 구매를 인증합니다.
...
`AdjustEvent.setDeduplicationId` 메서드를 호출하여 이벤트 중복 제거를 위해 추가적인 ID를 전달할 수 있습니다. Adjust가 SDK로부터 중복 이벤트를 수신하면 이를 무시합니다.
<tabs> <tab title="Kotlin" sync="kotlin"> 
...
기본 설정에 따라 Adjust SDK는 최대 **10** 개의 고유한 중복 제거 ID를 지원합니다. `AdjustConfig.setEventDeduplicationIdsMaxSize` 메서드를 호출하여 이 설정을 구성할 수 있습니다.
<tabs> <tab title="Kotlin" sync="kotlin"> 
...
본 예시에서는 사용자가 버튼을 조작할 때마다 `g3mfiw`라는 토큰으로 이벤트를 기록하는 방법을 보여줍니다. 이 함수는 `setDeduplicationId` 메서드를 사용하여 `orderId`를 `5e85484b-1ebc-4141-aab7-25b869e54c49`로 설정합니다.
<tabs> <tab title="Kotlin" sync="kotlin"> 
...
```txt
Path:      /event
ClientSdk: android$ANDROID_V5_VERSION
Parameters:
  environment      sandbox
  event_count      3
  event_token      abc123
...
</CodeBlock> <tabs> <tab title="Kotlin" sync="kotlin"> 
...
ID를 설정하려면 `setOrderId` 메서드를 호출하고 트랜잭션 ID를 `String` 인수로 전달합니다.
<tabs> <tab title="Kotlin" sync="kotlin"> 
