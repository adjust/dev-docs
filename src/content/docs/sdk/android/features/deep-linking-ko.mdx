...
4. [Android 앱 링크](https://help.adjust.com/en/article/set-up-android-app-links)의 경우, `intent` 필터를 추가하여 앱이 처리할 수 있는 URL을 지정합니다. `intent` 필터에 `android:autoVerify="true"` 어트리뷰션과 함께 `data` 요소를 포함합니다.
...
이제 앱이 URI 스킴과 Android 앱 링크를 모두 처리할 수 있습니다. 사용자가 선언된 URI 스킴 또는 Android 앱 링크와 일치하는 링크를 클릭하면 앱이 자동으로 실행됩니다.

사용자가 **스킴 이름** 을 포함하는 `deep_link` 파라미터가 포함된 링크를 클릭하면 이 활동이 실행됩니다.
