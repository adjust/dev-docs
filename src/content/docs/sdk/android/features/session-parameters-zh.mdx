...
Adjust SDK 会在应用打开时启动。如果您想通过会话参数发送的数据在应用打开时不可用，那么则可以延迟 SDK 启动。为此，以秒为单位将延迟时间发送至 config 对象的 [`setDelayStart`](/en/sdk/android/configuration#delay-the-start-of-the-sdk)方法。
<tabs> <tab title="Kotlin" sync="kotlin"> 
