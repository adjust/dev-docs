...
Adjust SDKは、`initSdk`または`onNewIntent`メソッドを使ってアクティビティのintentオブジェクト内にディープリンク情報を配信します。アプリが起動し、これらのメソッドのいずれかが起動すれば、ディープリンクコンテンツにアクセスできます。その後、アプリの他の部分でこの情報をアクセスできます。
...
</CodeBlock> <codeblock title="メソッドシグネチャー"> 
