...
[Android アプリリンク](https://help.adjust.com/en/article/set-up-android-app-links)には、 `AndroidManifest.xml`ファイルに`intent`フィルターを追加して、アプリが処理できるURLを指定します。`intent`フィルターに、 `data`要素を`android:autoVerify="true"`アトリビュートとともに含めます。
...
[最後に解析されたリンクを取得](get-the-last-resolved-link)
--------------------------------------------
...
`Adjust.getLastDeeplink()`メソッドを呼び出すことで、`Adjust.processDeeplink()`もしくは[`Adjust.processAndResolveDeepLink()`メソッド](/en/sdk/android/features/short-links)でresolve（解析および変換）された最後のディープリンクURLを返すことができます。このメソッドは、最後に解析されたディープリンクをディープリンクオブジェクトとして返します。
<tabs> <tab title="Kotlin" sync="kotlin"> 
