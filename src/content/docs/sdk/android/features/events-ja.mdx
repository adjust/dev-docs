...
`AdjustEvent.setDeduplicationId`メソッドを呼び出すと、イベント重複排除のためのオプション識別子をパスすることができます。AdjustはSDKから重複したイベントを受信すると、それらを排除します。
...
デフォルトでは、Adjust SDKは最大 **10** のユニークIDをサポートしています。`AdjustConfig.setEventDeduplicationIdsMaxSize`メソッドを呼び出すことで、この設定を構成できます。
