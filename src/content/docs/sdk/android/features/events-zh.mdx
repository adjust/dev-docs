...
您可以通过调用`AdjustEvent.setDeduplicationId`方法来传递可选的标识符，用于事件去重。Adjust 从 SDK 接收到重复事件时，会忽略这些重复事件。
<tabs> <tab title="Kotlin" sync="kotlin"> 
...
Adjust SDK 默认支持最多 **10** 个唯一去重 ID。您可以通过调用`AdjustConfig.setEventDeduplicationIdsMaxSize`方法来调整此设置。
<tabs> <tab title="Kotlin" sync="kotlin"> 
