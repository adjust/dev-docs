...
Adjust SDK 会使用 `onCreate` 或 `onNewIntent` 方法，传递活动 intent 对象中的深度连接信息。深度链接位于`intent`对象的`data`属性。应用启动，且上述方法之一被调用后，您就可以读取深度链接内容了。调用`processDeeplink`方法来解析将用户深度链接至应用的 Adjust 短链接 URL 。
<tabs> <tab title="Kotlin" sync="kotlin"> 
