...
4. 对于[安卓应用链接](https://help.adjust.com/en/article/set-up-android-app-links)，添加`intent`过滤器，指定应用可以处理的 URL。在`intent`过滤器中纳入带`android:autoVerify="true"`属性的`data`元素。
...
您的应用现在可以同时处理 URI 方案和安卓应用链接了。当用户点击的链接与已声明的 URI scheme 或安卓应用链接相匹配时，您的应用就会自动打开。
