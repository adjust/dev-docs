...
2. デバイスに関する全ての情報を消去するには、テストコンソールを使用してデバイスに関する情報を削除します。[Adjust Insightsアプリ](https://play.google.com/store/apps/details?id=com.adjust.insights)で取得したアプリのアルゴリズムと広告IDをパスすることで、forget\_deviceエンドポイントを使用することも可能です。次の広告IDはいずれも使用できます：`adid`、`android_id`、`gps_adid`、`imei`、`oaid`。
