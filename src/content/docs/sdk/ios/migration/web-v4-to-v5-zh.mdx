...
在 SDK v4 中，您可以使用 Adjust 网桥，通过布尔参数调用`AdjustConfig`实例的`setLinkMeEnabled`方法来启用[Adjust LinkMe](https://help.adjust.com/en/article/linkme) 。
</MinorVersion> 
...
在 SDK v4 中，您可以通过布尔参数在`AdjustConfig`实例上调用`setCoppaCompliantEnabled`方法来设置[COPPA 合规性](/en/sdk/android/features/privacy#coppa-compliance)。
</MinorVersion> 
