...
SDK v4では、ブーリアン引数で`AdjustConfig`インスタンスの`setLinkMeEnabled`メソッドを呼び出すことで、Adjust Web bridgeを使用して[Adjust LinkMe](https://help.adjust.com/en/article/linkme)を有効にできます。
</MinorVersion> 
...
SDK v4では、 [COPPAへの準拠](/en/sdk/android/features/privacy#coppa-compliance)は、ブーリアン引数で`AdjustConfig`インスタンスの`setCoppaCompliantEnabled`メソッドを呼び出すことで設定されます。
</MinorVersion> 
...
</MinorVersion> <codeblock title="ADIDゲッター"> 
