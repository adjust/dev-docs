---
title: "iOS Web bridge v5 마이그레이션 가이드"
description: "본 가이드를 참조하여 iOS 웹 브릿지를 SDK v4에서 SDK v5로 업그레이드하시기 바랍니다."
...
SDK v4에서 [COPPA 준수](/en/sdk/android/features/privacy#coppa-compliance) 는 부울 인수와 함께 `AdjustConfig` 인스턴스에서 `setCoppaCompliantEnabled` 메서드를 호출하여 설정됩니다.
...
### 요청 ATT 승인 상태 \{\#request\-att\-authorization\-status\}
...
SDK v4에서는 `AdjustConfig` 인스턴스의 `setLinkMeEnabled` 메서드를 부울 인수와 함께 호출하여 Adjust 웹 브릿지를 통해 [Adjust LinkMe](https://help.adjust.com/en/article/linkme) 를 활성화할 수 있습니다.
