...
### SDK Bilib 라이브러리 \{\#signature\-setup\}를 구성합니다.
...
<tabs> <tab title="Swift" sync="swift"> <codeblock title="ADID 게터"> 
...
SDK v4는 SDK 트래픽 서명에 사용할 `ADJConfig` 인스턴스에서 `setAppSecret` 메서드를 호출하여 앱 시크릿을 설정하는 것을 지원합니다.
<tabs> <tab title="Swift" sync="swift"> 
