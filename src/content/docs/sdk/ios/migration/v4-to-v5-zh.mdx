...
In SDK v4, you can enable [Adjust LinkMe](https://help.adjust.com/zh/article/linkme) by calling the `[ADJConfig setLinkMeEnabled]` method with a `true` argument.
...
In SDK v4, [COPPA compliance](/zh/sdk/ios/features/privacy#coppa-compliance) is set using the `ADJConfig` class and is read during SDK initialization. This means that the value can't be updated once the SDK is initialized.
