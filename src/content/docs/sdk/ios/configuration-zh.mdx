...
-  `sendInBackground` \( **布尔** \)：设为 `true` 可启用后台请求发送；设为 `false` 可禁用后台请求发送。
...
-  `eventBufferingEnabled` \( **布尔** \)：设为 `true` 可启用事件；设为 `false` 可禁用事件缓冲。
...
By default, the Adjust SDK starts as soon as your app opens. If you want to send data that's not available at launch in [session parameters](/zh/sdk/ios/features/session-parameters), you can delay the start of the SDK. This can be helpful if you are sending information such as unique identifiers.
...
-  `enabled` \( **布尔** \)：设为 `true` 可启用离线模式；设为 `false` 可禁用离线模式。
...
Push tokens are used for [Audiences](https://help.adjust.com/zh/article/audiences) and client callbacks. They're also required for [Uninstall and reinstall tracking](https://help.adjust.com/zh/article/uninstalls-reinstalls).
...
-  `enabled` \( **布尔** \)：设为 `true` 可启用 SDK；设为 `false` 可禁用 SDK。
