...
versions:
   ...
   - label: v4
     value: v4
redirects:
   v4: /ko/sdk/ios/v4/plugins/facebook-pixel
---
...
`init`를 호출한 뒤 바로 설정해야 합니다. Facebook이 제공한 스니펫은 `track` 메서드를 포함합니다. 이 메서드를 사용하여 `init` 메서드를 호출한 뒤 페이지 뷰 이벤트를 기록할 수 있습니다. 뷰 이벤트를 기록하려면 `set`을 `init`과 `track` 사이에 호출해야 합니다.
