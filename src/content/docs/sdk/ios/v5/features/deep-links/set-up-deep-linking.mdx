...
slug: en/sdk/ios/features/deep-links/set-up-deep-linking
sidebar-position: 2
versions:
   - label: v5
     value: v5
   ...
   - label: v4
     value: v4
redirects:
   v4: /en/sdk/ios/v4/features/deep-links/set-up-deep-linking
---
  ...
  func adjustDeferredDeeplinkReceived(_ deeplink: URL?) -> Bool {
    if let incomingLink = deeplink {
      if UserDefaults.standard.bool(forKey: "HasCompletedOnboarding") {
        ...
        UserDefaults.standard.set(incomingLink.absoluteString, forKey: "lastDeferredLink")
      }
    }
    ...
    // Otherwise, return false to prevent SDK from opening the deep link immediately.
    return false
  }
```
...
class ViewController: UIViewController {
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    if !hasCompletedOnboarding {
      ...
      // Check for deferred deep link that arrived during onboarding
...
class DeeplinkHandler {
 static func handleDeeplink(_ incomingLink: URL) {
   ...
   Adjust.processAndResolveDeeplink(deeplink) { processedLinkString in
     ...
     var path: String
     if let scheme = processedLink.scheme, scheme == "http" ||
         scheme == "https" {
...
+ (void)handleDeeplink:(NSURL *)incomingLink {
 ...
 [Adjust processAndResolveDeeplink:deeplink
     withCompletionHandler:^(NSString *_Nonnull processedLinkString) {
