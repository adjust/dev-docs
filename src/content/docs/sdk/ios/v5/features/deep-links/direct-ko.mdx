...
* `ADJLinkResolution.resolveLink`: 마케팅팀이 Adjust의 링크 해석\(link resolution\) 솔루션이 필요한 경우에만 이 메서드를 호출합니다. 딥링크가 : 의 요소와 일치하는 도메인을 사용하는 경우, 이 메서드는 딥링크를 해석하려고 시도하여 해석된 링크를 반환합니다. 딥링크가 이 배열의 요소와 일치하지 않는 경우, 이 메서드는 원래의 딥링크를 전달하므로 모든 딥링크를 이 메서드로 전달할 수 있습니다.
