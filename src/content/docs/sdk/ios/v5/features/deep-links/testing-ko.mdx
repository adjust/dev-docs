...
versions: 
    ...
    - 
        ...
        value: "v4"
redirects: 
    v4: "/ko/sdk/ios/v4/features/deep-links/testing"
---
...
이 경우, 앱이 실행되고 딥링크 내용이 표시되어야 합니다.  앱에서 올바른 페이지가 표시되지 않는 경우 다음을 확인하시기 바랍니다.
...
* 앱이 딥링크를 수신한 메서드에서 `processDeeplink` 메서드를 호출하는지 확인합니다.
...
딥링크가 선택되면 앱은 Adjust SDK에서 `processDeeplink` 메서드를 호출하며, Xcode 로그에서 다음과 같은 입력 내용을 확인할 수 있어야 합니다.
...
4. [테스트용 콘솔](https://help.adjust.com/en/article/testing-console)을 열고, Adjust ADID를 입력한 뒤 **기기 데이터 보기** 를 선택합니다. `TrackerName`이 `Organic`으로 표시되어야 합니다.
