...
import TrackEvent from "@ios-signatures/Adjust/trackEvent.mdx"

This class contains information about events triggered in your application. You can send this information to Adjust's servers by passing an `ADJEvent` object to the <Tooltip>`trackEvent` method || <TrackEvent /></Tooltip>.
...
`eventToken` (NSString): A 6 character Adjust event token.
...
`transactionId` (NSString): A unique identifier used to deduplicate events.
...
`callbackId` (NSString): A custom identifier for your event object.
