...
`logLevel` (ADJLogLevel): The level of logging output by the SDK.
...
`defaultTracker` (NSString): The token to which all preinstalled sessions are attributed.
...
`delegate` (NSObject): The delegate function that's called when attribution changes.
...
`externalDeviceId` (NSString): The external device ID associated with the device.
...
`allowIdfaReading` (BOOL): Whether to allow the SDK to read the device's IDFA.
...
`allowAdServicesInfoReading` (BOOL): Whether to allow the SDK to read AdServices information.
