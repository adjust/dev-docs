...
上述解决方案不支持[合作伙伴参数](https://help.adjust.com/en/article/record-events-ios-sdk#add-partner-parameters)。如果您需要向渠道发送带有自定义参数的事件，用于动态再营销等目的，请在 Segment SDK 和 Adjust SDK 中设置事件。请勿在 Segment 控制面板中将这些事件关联到 Adjust 端。
</Callout> 
...
2. 为每个要跟踪的应用内事件创建跟踪调用 \([iOS](https://segment.com/docs/connections/sources/catalog/libraries/mobile/ios/#track)\)。
...
2. 将下列参数以[会话合作伙伴参数](https://help.adjust.com/en/article/session-parameters-ios-sdk#session-partner-parameters)形式发送至 Adjust SDK。只有启用 [Delay Start](https://help.adjust.com/en/article/session-parameters-ios-sdk#delay-start) \(延迟启动\)，这些参数才会在安装或会话开始时可用。

* `anonymous_id` \- Adjust 向 Segment 发送回传所必需。您可以[从 Segment SDK 调取 anonymous\_id 值](https://segment.com/docs/connections/sources/catalog/libraries/mobile/ios/#anonymousid)。
...
1. 集成 [Segment iOS SDK](https://segment.com/docs/connections/sources/catalog/libraries/mobile/ios/)。
2. 在 Segment SDK 中添加 Adjust 集成 \([iOS](https://segment.com/docs/connections/destinations/catalog/adjust/#ios)\)。
3. 向您的应用添加[额外 iOS 框架](https://help.adjust.com/en/article/get-started-ios-sdk#add-ios-frameworks)。
4. 为每个要跟踪的应用内事件创建跟踪调用 \([iOS](https://segment.com/docs/connections/spec/track/)\)。
