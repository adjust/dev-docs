...
* `environment` \( **NSString** \): SDK를 실행할 환경입니다. 테스트를 위해 샌드박스 모드에서 SDK를 실행하려면 `ADJEnvironmentSandbox`를 전달합니다. 릴리즈를 위해 프로덕션 모드에서 SDK를 실행하려면 `ADJEnvironmentProduction`를 전달합니다.
* `allowSuppressLogLevel` \( **BOOL** \): 모든 로깅의 비활성화 여부. `true`로 설정하여 로깅을 비활성화하거나 `false`로 설정하여 로깅을 활성화합니다.
...
* `externalDeviceId` \( **NSString** \): 외부 디바이스 ID. 이 값은 **대소문자를 구분** 합니다. 외부 디바이스 ID를 불러오기한 경우, 전달한 값이 불러온 값과 일치하는지 확인하시기 바랍니다.
...
* `defaultTracker` \( **NSString** \): 사전 설치된 설치 내용을 기록할 [Adjust 링크 토큰](https://help.adjust.com/en/article/links#adjust-link-token)입니다.
...
* `needsCost` \( **BOOL** \): 비용 데이터 전송을 활성화하려면 `true`, 비용 데이터 전송을 비활성화하려면 `false`로 설정합니다.
...
* `sendInBackground` \( **BOOL** \): 백그라운드 전송을 활성화하려면 `true`, 백그라운드 전송을 비활성화하려면 `false`로 설정합니다.
...
* `eventBufferingEnabled` \( **BOOL** \): 이벤트 버퍼링을 활성화하려면 `true`, 이벤트 버퍼링을 비활성화하려면 `false`로 설정합니다.
...
시작 지연을 설정하려면 config 인스턴스의 `delayStart` 속성에 **Boolean** 값을 할당합니다.
...
* `enabled` \( **BOOL** \): 오프라인 모드를 활성화하려면 `true`, 오프라인 모드를 비활성화하려면 `false`로 설정합니다.
...
* `enabled` \( **BOOL** \): SDK를 활성화하려면 `true`, SDK를 비활성화하려면 `false`로 설정합니다.
