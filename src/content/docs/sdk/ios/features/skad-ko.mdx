...
| 인수 \(Arguments\) | 데이터 유형  |                                                                         설명                                                                          |
|--------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| `fineValue`        | Integer | Adjust의 서버가 전송하는 전환값                                                                                                                                |
| `coarseValue`      | String  | coarse\-grained 전환값. 앱의 설치 수가 Privacy Threshold를 충족하지 못하는 경우에 사용됩니다.<ul><li>`low`</li><li>`medium`</li><li>`high`</li></ul>                        |
| `lockWindow`       | Integer | 전환값 윈도우가 종료되기 전에 포스트백을 전송할지 여부. `1`은 전환값 윈도우 종료 전에 포스트백이 전송됨을 의미합니다.<br/>SKAdNetwork 4\.0 포스트백에서는 `0`, 그 이전 SKAdNetwork 버전에서는 `nil`로 기본 설정되어 있습니다. |
