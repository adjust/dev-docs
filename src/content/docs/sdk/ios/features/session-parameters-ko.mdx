...
글로벌 [파트너 파라미터](https://help.adjust.com/en/article/data-sharing-ad-network#map-parameters)를 추가하여 네트워크 파트너에게 추가 정보를 전송할 수 있습니다.
...
`NSString` 키\-값 인수와 함께 `addSessionCallbackParameter` 메서드를 호출하여 이벤트에 콜백 파라미터를 추가합니다. 이 메서드를 여러 번 호출함으로써 여러 파라미터를 추가할 수 있습니다.
<tabs> <tab title="Swift" sync="swift"> 
...
`NSString` 키\-값 인수와 함께 `addSessionPartnerParameter` 메서드를 호출하여 세션의 파트너 파라미터를 전송합니다. 이 메서드를 여러 번 호출함으로써 여러 파라미터를 추가할 수 있습니다.
<tabs> <tab title="Swift" sync="swift"> 
...
Adjust SDK는 앱이 시작된 직후에 작동을 시작합니다. 시작할 때 사용할 수 없는 데이터를 세션 파라미터로 전송하려면 SDK의 시작을 지연시킬 수 있습니다. 이를 위해 config 객체의 [`delayStart` 속성](/en/sdk/ios/configuration#delay-the-start-of-the-sdk)에 지연 시간\(초\)을 할당합니다.
<tabs> <tab title="Swift" sync="swift"> 
