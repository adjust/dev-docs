...
* [ ]  [Adjustでアプリを追加していること。](https://help.adjust.com/en/article/app-setup)
* [ ]  必要なディープリンクデータを全て取得していること。

[ユニバーサルリンクをアプリに設定](set-up-universal-links-in-your-app)
------------------------------------------------------
...
[カスタムURLスキームを使用したディープリンクを設定](set-up-deep-links-with-a-custom-url-scheme)
------------------------------------------------------------------------
...
4. `deeplink_js=1`パラメーターを、エンコードされたディープリンクを含むトラッカーURLに追加します。これにより、AdjustシステムでiOSカスタムURLスキームが使用されるようになります。例： `https://app.adjust.com/abc123?deeplink_js=1&deeplink=%3A%2F%2Fsummer-clothes%3Fpromo%3Dbeach`
