...
ユーザーがサブスクリプションを購入した地域を記録することができます。これを行うには、subscriptionオブジェクトの`setSalesRegion`メソッドを呼び、国コードを`NSString`としてパスします。これは、[`Storefront`](https://developer.apple.com/documentation/storekit/storefront)オブジェクトの[`countryCode`](https://developer.apple.com/documentation/storekit/storefront/3792000-countrycode)としてフォーマットされる必要があります。
<tabs> <tab title="Swift" sync="swift"> 
