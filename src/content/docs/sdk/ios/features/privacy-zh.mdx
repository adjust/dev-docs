...
<tabs> <tab title="Swift" sync="swift"> <codeblock title="India URL strategy"> 
...
</CodeBlock> </Tab> <tab title="Objective-C" sync="objc"> <codeblock title="India URL strategy"> 
...
|           值            |                        描述                        |
|------------------------|--------------------------------------------------|
| `ADJDataResidencyEU`   | 将数据驻留地区设为欧盟                                      |
| `ADJDataResidencyTR`   | 将数据驻留地区设为土耳其                                     |
| `ADJDataResidencyUS`   | 将数据驻留地区设为美利坚合众国                                  |
| `ADJUrlStrategyChina`  | 将 URL 测试指定到 `app.adjust.world` 域                 |
| `ADJUrlStrategyCn`     | 将 URL 测试指定到 `app.adjust.cn` 域                    |
| `ADJUrlStrategyCnOnly` | 将 URL 策略指向`app.adjust.cn`域名，但请求失败时，不会使用其他域名作为后备。 |
| `ADJUrlStrategyIndia`  | 将 URL 测试指定到 `app.adjust.net.in` 域                |
...
要让您的应用符合 COPPA \(儿童在线隐私保护法\) 规定，请调用 `[ADJConfig setCoppaCompliantEnabled]` 方法。此方法会进行如下操作：
