...
Adjustは各デバイスに固有のAdjust Device ID（ADID）を生成します。`adid`メソッドを呼び出すと、このIDを **string** として返します。
...
IDFA（ID for Advertisers）は、Appleデバイスの端末固有の識別子です。`idfa`メソッドを呼び出すと、このIDを **string** として返します。
