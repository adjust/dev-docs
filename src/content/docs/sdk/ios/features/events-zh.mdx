...
* `eventToken` \(`NSString`\)：您的 Adjust [事件识别码](https://help.adjust.com/en/article/add-events#manage-your-events)。
...
</Tab> </Tabs> <codeblock title="Event log" collapse="6-40"> 
...
</Tab> </Tabs> <codeblock title="Event log" highlight="{range: 7}"> 
...
如果您已启用[购买验证](https://help.adjust.com/en/article/purchase-verification)，则必须随购买事件一同发送额外信息用于验证。Adjust 服务器收到事件对象中的信息后，会将此信息转发至 Apple 验证购买。
...
</CodeBlock> </Tab> </Tabs> <codeblock title="Event log" highlight="{range: 4}"> 
...
</Tab> </Tabs> <codeblock title="Event log" highlight="{range: 4}"> 
...
</Tab> </Tabs> <codeblock title="Event log"> 
