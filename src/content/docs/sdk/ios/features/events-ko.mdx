...
* `eventToken` \(`NSString`\): Adjust [이벤트 토큰](https://help.adjust.com/en/article/add-events#manage-your-events)
...
</Tab> </Tabs> <codeblock title="이벤트 로그" collapse="6-40"> 
...
</Tab> </Tabs> <codeblock title="이벤트 로그" highlight="{range: 7}"> 
...
### [결제 검증](purchase-verification)

[구매 검증](https://help.adjust.com/en/article/purchase-verification)을 활성화한 경우, 검증을 위해 구매 이벤트와 함께 추가 정보를 전송해야 합니다. Adjust의 서버는 이벤트 객체에서 이 정보를 수신하면 이를 Apple에 전달하여 구매를 검증합니다.
...
</CodeBlock> </Tab> </Tabs> <codeblock title="이벤트 로그" highlight="{range: 4}"> 
...
</Tab> </Tabs> <codeblock title="이벤트 로그" highlight="{range: 4}"> 
...
</Tab> </Tabs> <codeblock title="이벤트 로그"> 
