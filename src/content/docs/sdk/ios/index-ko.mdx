...
* `appToken`: 귀하의 [앱 토큰](https://help.adjust.com/en/article/app-settings#view-your-app-token).
...
Adjust SDK를 설정해 앱 정보를 기록할 수 있으므로 이제 테스트할 차례입니다. Adjust는 앱 테스트에 도움이 되는 [테스팅 콘솔](https://help.adjust.com/en/article/testing-console)과 [Device API](https://help.adjust.com/en/article/device-api)를 제공합니다.

[테스트 가이드](https://help.adjust.com/en/article/test-the-adjust-sdk)를 따라 Adjust가 앱에서 예상값을 수신하는지 확인합니다.
...
</Tab> <tab title="프레임워크로 추가" sync="framework"> <callout type="important"> 

정적 프레임워크의 배포는 [프라이버시 매니페스트](https://help.adjust.com/en/article/submit-privacy-details-to-apple#privacy-manifest) 를 포함하지 않기 때문에 버전 4\.38\.0부터 지원이 중단됩니다.
...
* `AdjustSdkStatic.framework.zip`: iOS 7 이하를 실행하는 장치
* `AdjustSdkDynamic.framework.zip`: iOS 8 이상을 실행하는 장치
...
</Tab> </Tabs> </Tab> <tab title="프레임워크 가져오기" sync="framework"> 
