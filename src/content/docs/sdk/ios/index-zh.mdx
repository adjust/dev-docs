...
The distribution of static frameworks is deprecated from version 4\.38\.0, as a result of not including the [Privacy Manifest](https://help.adjust.com/zh/article/submit-privacy-details-to-apple#privacy-manifest).
...
-  `appToken`: Your [app's token](https://help.adjust.com/zh/article/app-settings#view-your-app-token).
-  `environment`: SDK 运行的环境。将其发送至`ADJEnvironmentSandbox`。
...
Now that you've configured the Adjust SDK to record information about your app, it's time to test it. Adjust offers a [testing console](https://help.adjust.com/zh/article/testing-console) and a [Device API](https://help.adjust.com/zh/article/device-api) to help you test your app.

Follow the [testing guide](https://help.adjust.com/zh/article/test-the-adjust-sdk) to make sure Adjust receives the expected values back from your app.
...
-  `logLevel`: Choose a [logging level](/zh/sdk/ios/configuration#set-your-logging-level), or disable logging completely by passing an `allowSuppressLogLevel` argument in your config object.
