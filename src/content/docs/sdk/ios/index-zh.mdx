...
SDK v5 包含[SDK 签名库](https://help.adjust.com/en/article/sdk-signature)。签名保护功能默认处于禁用状态。要启用此功能，您需要[执行签名验证](https://help.adjust.com/en/article/sdk-signature#enforce-signature-validation)。
...
完成配置，能通过 Adjust SDK 记录应用相关信息后，您就可以进行测试了。Adjust 为您提供[测试控制台](https://help.adjust.com/en/article/testing-console)和[设备 ID](https://help.adjust.com/en/article/device-api)，方便您测试应用。

请按照[测试指南](https://help.adjust.com/en/article/test-the-adjust-sdk)中的说明操作，确保您的应用能将预期值发送给 Adjust。
...
</Tab> <tab title="添加为框架" sync="framework"> <callout type="important"> 

由于缺失[隐私清单](https://help.adjust.com/en/article/submit-privacy-details-to-apple#privacy-manifest)，静态框架分布将从自 4\.38\.0 版本起停用。
</Callout> 
...
</Tab> </Tabs> </Tab> <tab title="框架导入" sync="framework"> 
