...
-  [x] Google PlayやHuawei App Galleryなどのアプリストアで、ストアWebサイト上のアプリページURLに使用できる`referrer`パラメーター。リファラーパラメーターの設定方法は次のとおりです。
   -  When a user clicks on an Adjust link, the Adjust server passes a unique identifier called `reftag`. This identifier is assigned to the click and into the referrer parameter. To know more about reftag, visit [Reftag](https://help.adjust.com/ja/article/reftag).
...
| **AppTrackingTransparency.framework** | This framework is needed in iOS 14 and later for the SDK to be able to wrap he user ATT consent dialog and access the user’s consent response. | アプリが「子供向け」カテゴリーをターゲットにしている場合は、このframeworkは実装しないでください。 |
