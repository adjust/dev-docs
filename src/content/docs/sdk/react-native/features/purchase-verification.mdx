...
You can access the purchase verification status by using a callback. Results are returned as `AdjustPurchaseVerificationInfo` objects containing the following properties:
...
2. Call the `Adjust.verifyAndTrackPlayStorePurchase` method with the following arguments:
   ...
   -  `callback` (`function`): A delegate callback function that receives an `AdjustPurchaseVerificationInfo` object as an argument.
...
2. Call the `Adjust.verifyPlayStorePurchase` method with the following arguments:
   ...
   -  `verificationResultCallback` (`function`): A delegate callback function that receives an `AdjustPurchaseVerificationInfo` object as an argument.
...
See [Send event information](/en/sdk/react-native/features/events) for more information on the `AdjustEvent` class.
