...
ID를 설정하려면 `setDeduplicationId` 메서드를 호출하고 트랜잭션 ID를 `string` 인수로 전달합니다.
...
### Set a deduplication limit \{\#set\-a\-deduplication\-limit\}
...
식별된 중복 이벤트에 대해 목록에 추가할 수 있는 중복 제거 ID의 수에 맞춤 제한을 설정할 수 있습니다. 기본값은 10으로 설정되어 있습니다.

맞춤 제한을 설정하려면 `setEventDeduplicationIdsMaxSize` 메서드를 호출하고 새로운 제한을 `number` 인수로 전달합니다.
