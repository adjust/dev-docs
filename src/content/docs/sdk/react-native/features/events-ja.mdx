...
識別子を設定するには、`setDeduplicationId`メソッドを呼び出し、`string`引数として重複IDをパスしてください。
...
### 重複排除の制限を設定する \{\#set\-a\-deduplication\-limit\}
...
重複したイベントを識別するために、リストに追加できる重複IDの数をカスタム制限することができます。デフォルトでは制限は10です。

カスタム制限を設定するには、`setEventDeduplicationIdsMaxSize`メソッドを呼び出し、`number`引数として新しい制限をパスしてください。
