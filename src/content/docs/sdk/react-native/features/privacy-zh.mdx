...
-  `urlStrategyDomains` \(`any`\)：接收您所发送 SDK 流量的数据驻留国家/地区或终端。
...
要让您的应用符合 COPPA \(儿童在线隐私保护法\) 规定，请调用 `enableCoppaCompliance` 方法。此方法会进行如下操作：
...
如需将应用标记为儿童应用，请调用`enablePlayStoreKidsCompliance`方法。
