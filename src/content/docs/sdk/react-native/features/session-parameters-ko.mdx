...
Adjust SDK는 앱이 시작된 직후에 작동을 시작합니다. 시작할 때 사용할 수 없는 데이터를 세션 파라미터로 전송하려면 SDK의 시작을 지연시킬 수 있습니다. 이를 위해 config 객체의 [`setDelayStart` 메서드](/ko/sdk/react-native/configuration#delay-the-start-of-the-sdk)로 지연 시간\(초\)을 전달합니다.
