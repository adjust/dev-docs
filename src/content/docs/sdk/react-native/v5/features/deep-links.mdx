...
To reattribute your user, you need to call the `Adjust.processDeeplink` method when the app receives deep link content. The Adjust SDK then looks for new attribution data within the deep link. If the SKD finds new information, it forwards the information to Adjust's servers for reattribution.
