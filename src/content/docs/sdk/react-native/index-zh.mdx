...
* 选择您的环境： 
  * 如要测试应用并发送测试数据，请使用`AdjustConfig.EnvironmentSandbox`。请在控制面板中启用 [sandbox 模式](https://help.adjust.com/en/article/filter-data#sandbox)，查看测试数据。
...
* [x]  Google Play 和华为 App Gallery 等应用商店在其商店网站应用页面 URL 中接受的 `referrer` 参数。referrer 参数按以下方式填充： 
  * 用户点击 Adjust 链接时，Adjust 服务器发送一个独一无二的识别码，即 `reftag`。该识别码被指配给此点击，并被添加至 referrer 参数。要进一步了解 reftag，请访问[Reftag](https://help.adjust.com/en/article/reftag) 页面。
...
通过 Adjust Meta Install Referrer 插件，Adjust SDK 可以支持 [Meta Install Referrer](https://developers.facebook.com/docs/app-ads/meta-install-referrer)。[要了解集成细节，请参考 React Native Meta 安装 Referrer 插件。](/en/sdk/react-native/plugins/meta-referrer-plugin)
