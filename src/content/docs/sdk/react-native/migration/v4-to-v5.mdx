...
In SDK v4, you can call the `setShouldlaunchDeeplink` method on your `adjustConfig` instance with the value `true` to enable the SDK to open deep links.
...
In SDK v5, you need to call the `isDeferredDeeplinkOpeningEnabled` method on your `adjustConfig` instance to enable the SDK to open deep links. The default state is `true`.
...
In SDK v4, you can call the `coppaCompliantEnabled` method on your `adjustConfig` instance with the cvalue `true` to enable COPPA compliance.
...
In SDK v5, you need to assign a callback function to the `setSkanUpdatedCallbackListener` method of your `adjustConfig` object.
