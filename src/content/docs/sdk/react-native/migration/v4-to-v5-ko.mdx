...
SDK v4에서는 `adjustConfig` 인스턴스의 `setLinkMeEnabled` 메서드를 `true` 값과 함께 호출하여 [Adjust LinkMe](https://help.adjust.com/en/article/linkme) 를 활성화할 수 있습니다.

```jsx
adjustConfig.setLinkMeEnabled(true);
```

SDK v5에서는 `adjustConfig` 인스턴스에서 `enableLinkMe` 메서드를 호출하여 [Adjust LinkMe](https://help.adjust.com/en/article/linkme) 를 활성화해야 합니다. 디폴트 상태는 `false` 입니다.
...
### 딥링크 콜백 수신기 \{\#deeplink\-callback\-listener\}
...
SDK v4에서는 `adjustConfig` 인스턴스의 `setPlayStoreKidsAppEnabled` 메서드를 `true` 값과 함께 호출하여 앱을 [Play Store Kids 앱](/en/sdk/react-native/features/privacy#play-store-kids-apps-android-only) 으로 표시할 수 있습니다. 이 속성은 SDK 초기화 중에 읽히기 때문에 SDK가 초기화된 후에는 해당 값을 업데이트할 수 없습니다.
