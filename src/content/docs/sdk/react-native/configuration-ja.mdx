...
* `environment` （ **環境** ）：SDKを実行したい環境。テスト用のサンドボックスモードでSDKを実行するには`AdjustConfig.EnvironmentSandbox`をパスします。リリース用の本番モードでSDKを実行するには`AdjustConfig.EnvironmentProduction`をパスします。
