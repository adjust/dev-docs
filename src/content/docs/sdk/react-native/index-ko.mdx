...
-  [x] Google Play 및 Huawei App Gallery와 같은 앱 스토어가 스토어 웹사이트의 앱 페이지 URL에서 허용하는 `referrer` 파라미터입니다. 리퍼러 파라미터가 생성되는 방법은 다음과 같습니다.
   -  사용자가 Adjust 트래커를 클릭하면 Adjust 서버는 `reftag`라는 고유 식별자를 전달합니다. 이 식별자는 클릭과 리퍼러 파라미터에 할당됩니다. reftag에 관한 자세한 정보는 [Reftag](https://help.adjust.com/ko/article/reftag)를 참고하시기 바랍니다.
