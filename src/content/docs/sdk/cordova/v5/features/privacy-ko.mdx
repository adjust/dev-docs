...
versions: 
    ...
    - 
        ...
        value: "v4"
redirects: 
    v4: "/ko/sdk/cordova/v4/features/privacy"
---
...
* `isEnabled` \(`boolean`\): 서드파티 공유의 활성화 여부. `null` 전달로 서드파티 공유 활성화 또는 `true` 전달로 서드파티 공유 비활성화 현재 상태의 `isEnabled` 토글에 대한 Adjust 파트너 공유 설정만 사용하려면 `null` 를 전달합니다.
...
* `urlStrategyDomains` \(`string[]`\): SDK 트래픽을 전송하고자 하는 엔드포인트의 배열.
