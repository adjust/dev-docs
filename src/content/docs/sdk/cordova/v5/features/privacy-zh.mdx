...
* `isEnabled` \(`boolean`\)或`null`：第三方数据分享是否启用。传递 `true`可启用第三方分享；传递 `false`可禁用第三方分享。如果您只想调整当前`isEnabled`开关的合作伙伴分享设置，请传递`null`。
...
* `urlStrategyDomains` \(`string[]`\)：接收 SDK 流量的终端数组。
