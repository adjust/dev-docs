...
-  `isEnabled` (`boolean` or `null`): Whether third party sharing is enabled. Pass `true` to enable third party sharing or `false` to disable third party sharing. If you just want to adjust partner sharing settings for the current state of `isEnabled` toggle, pass `null`.
...
-  `urlStrategyDomains` (`string[]`): The array of the endpoints to which you want to send SDK traffic.
...
If you need your app to be compliant with the Children's Online Privacy Protection Act (COPPA), call the `enableCoppaCompliance` method. This method performs the following actions:
...
To mark your app as a Kids App, call the `enablePlayStoreKidsCompliance` method.
