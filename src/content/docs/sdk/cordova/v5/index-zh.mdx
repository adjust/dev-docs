...
Adjust SDK 默认支持[Google Play Referrer API](https://developer.android.com/google/play/installreferrer)。对于安卓端，Huawei Referrer API 和 Meta Install Referrer 可作为插件使用。请参考相关文档，为您的安卓应用设置插件。

* [华为 Referrer API](/en/sdk/android/plugins/huawei-referrer-plugin)
* [Meta Install Referrer](/en/sdk/android/plugins/meta-referrer-plugin)

#### Meta referrer 集成\{\#meta\-referrer\-integration\}

要启用 Meta Install Referrer，您需要在`AdjustConfig`实例中设置 Meta 应用 ID。为此请进行下列操作：
...
2. 将您的应用 ID 传递至`AdjustConfig`实例的`setFbAppId`方法。
...
SDK v5 包含 [SDK 签名库](https://help.adjust.com/en/article/sdk-signature)。请按照[iOS](/en/sdk/ios/integrations/signature-library#test-your-app)和[安卓](/en/sdk/android/integrations/signature-library#test-your-app)测试指南操作，确保集成正确运行。
