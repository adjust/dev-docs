...
如果您想在商业分析中使用外部设备 ID，则可以将其作为[会话回传参数](/en/sdk/web/features/global-params)发送。
...
如果您的应用已预安装在设备上，那么您可以配置一个默认链接识别码。当用户首次打开预安装应用时，安装就会被归因给默认链接识别码。请在 `Adjust.initSdk` 调用的 `defaultTracker` 属性中传送您的识别码。
...
您可以随时使用下列参数调用 `switchToOfflineMode` 方法，以开关离线模式：
