...
|        値        | データタイプ |                                      説明                                       |
|-----------------|--------|-------------------------------------------------------------------------------|
| `adid`          | 文字列    | デバイス固有のAdjust識別子                                                              |
| `tracker_token` | 文字列    | アトリビューションに関連付けられているキャンペーントークン                                                 |
| `tracker_name`  | 文字列    | キャンペーンリンクの名前                                                                  |
| `network`       | 文字列    | キャンペーンに関連付けられているネットワーク                                                        |
| `campaign`      | 文字列    | アトリビューションに関連付けられているキャンペーンの名前                                                  |
| `adgroup`       | 文字列    | アトリビューションに関連付けられているアドグループ                                                     |
| `creative`      | 文字列    | アトリビューションに関連付けられているクリエイティブ                                                    |
| `click_label`   | 文字列    | アトリビューションに関連付けられている[クリックラベル](https://help.adjust.com/en/article/user-rewards) |
| `state`         | 文字列    | アトリビューションの現在のステータス。`installed`または `reattributed`                              |
...
リファラーを設定するには、`setReferrer`メソッドを呼び出し、URLエンコードされた **文字列** 引数としてリファラーをパスします。
