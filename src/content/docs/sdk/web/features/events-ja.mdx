...
イベントを送信するには、`Adjust.trackEvent`メソッドを呼び出して、Adjustの[イベントトークン](https://help.adjust.com/en/article/add-events#manage-your-events)を引数としてパスします。
...
この例では、ユーザーがボタンを操作する度に *`g3mfiw`* のトークンでイベントを計測する方法を示します。この関数は、`deduplicationId`を *`5e85484b-1ebc-4141-aab7-25b869e54c49`* に設定します。
