...
export type EventParamsT = {|
   ...
   callbackParams?: Array<globalparamst id="sl-md0000000">,
   partnerParams?: Array<globalparamst id="sl-md0000000">,
|};
```
...
이벤트를 전송하려면 `Adjust.trackEvent` 메서드를 호출하고 Adjust [이벤트 토큰](https://help.adjust.com/en/article/add-events#manage-your-events)을 인수로 전달합니다.
...
본 예시에서는 사용자가 버튼을 조작할 때마다 *`g3mfiw`* 라는 토큰으로 이벤트를 기록하는 방법을 보여줍니다. 이 함수로 `deduplicationId`가 *`5e85484b-1ebc-4141-aab7-25b869e54c49`* 로 설정됩니다.
...
</CodeBlock> <codeblock highlight="{range: 26-29}"> 
...
```js
function trackEvent(params: EventParamsT): Promise<void id="sl-md0000000"> {
