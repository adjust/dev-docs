...
versions: 
    ...
    - 
        ...
        value: "v4"
redirects: 
    v5: "/ko/sdk/cocos2dx/features/events"
---
Adjust SDK가 제공하는 `AdjustEvent2dx` 객체는 이벤트 정보를 구성하고 이러한 정보를 앱에서 Adjust 서버로 전송하는 데 사용할 수 있습니다.

AdjustEvent2dx 객체 인스턴스화
-----------------------
...
Adjust SDK를 사용하여 이벤트 정보를 전송하려면 `AdjustEvent2dx` 객체를 인스턴스화해야 합니다. 이 객체에는 앱에서 이벤트가 발생할 때 Adjust로 전송되는 변수가 포함됩니다.

이벤트 객체를 인스턴스화하려면 새로운 `AdjustEvent2dx` 인스턴스를 생성하고 다음 파라미터를 전달합니다.

* `eventToken` \( **문자열** \): Adjust [이벤트 토큰](https://help.adjust.com/en/article/add-events#manage-your-events).
...
* `currency` \( **문자열** \): 이벤트 통화의 [ISO 4217 코드](https://www.iban.com/currency-codes).
...
* `transactionId` \( **String** \): 성공적으로 완료된 구매의 [`transactionIdentifier`](https://developer.apple.com/documentation/storekit/skpaymenttransaction/1411288-transactionidentifier) 값
* `productId` \( **String** \): 성공적으로 구매된 아이템의 제품 식별자
* `receipt` \( **String** \): 성공적으로 완료된 구매에 대한 정보가 포함된 [서명된 영수증](https://developer.apple.com/documentation/foundation/bundle/1407276-appstorereceipturl) \(base64로 암호화 **되지 않음** \)
...
* `productId` \( **String** \): 성공적으로 구매된 아이템의 제품 식별자 \([`SKU`](https://developer.android.com/reference/com/android/billingclient/api/SkuDetails#getSku())\)
* `purchaseToken` \( **String** \): 성공적으로 완료된 구매의 [`purchase token`](https://developer.android.com/reference/com/android/billingclient/api/Purchase#getPurchaseToken()) 값
...
식별자를 설정하려면 `setTransactionId` 메서드를 호출하고 트랜잭션 ID를 **문자열** 인수로 전달합니다.
...
본 예시에서는 사용자가 버튼을 조작할 때마다 `g3mfiw`라는 토큰으로 이벤트를 기록하는 방법을 보여줍니다. 이 함수는 `setOrderId` 메서드를 사용하여 `orderId`를 `5e85484b-1ebc-4141-aab7-25b869e54c49`로 설정합니다.
...
로그에서 `callback_params`를 확인하고 파라미터가 Adjust로 전송되었음을 확인할 수 있습니다.
...
로그에서 `partner_params`를 확인하고 파라미터가 Adjust로 전송되었음을 확인할 수 있습니다.
...
ID를 **문자열** 인수로 한 `setCallbackId` 메서드를 호출하여 이 식별자를 설정할 수 있습니다.
