...
유럽 연합의 GDPR\(General Data Protection Regulation, 개인정보보호 규정\) 17조는 사용자에게 잊혀질 권리를 부여합니다. Adjust 서버가 RTBF\(Right to be Forgotten, 잊혀질 권리\) 요청을 수신하면 Adjust는 사용자 데이터를 삭제합니다. 또한 SDK는 해당 앱의 장치에서 요청 전송을 중지합니다.
...
Adjust SDK를 사용하여 사용자의 서드파티 공유 설정 변경 내용을 기록할 수 있습니다. 서드파티 공유 설정은 `AdjustThirdPartySharing2dx` 클래스를 사용하여 구성됩니다.

### AdjustThirdPartySharing2dx 객체 인스턴스화
...
Adjust SDK와의 서드파티 공유를 활성화 또는 비활성화하려면 `AdjustThirdPartySharing2dx` 객체를 인스턴스화해야 합니다. 이 객체에 Adjust의 서드파티 공유 처리 방식을 제어하는 변수가 포함됩니다.

서드파티 공유 객체를 인스턴스화하려면 새로운 `AdjustThirdPartySharing2dx` 인스턴스를 생성하고 다음 파라미터를 전달합니다.
...
`AdjustThirdPartySharing2dx` 객체를 인스턴스화한 후에 `AdjustThirdPartySharing2dx` 인스턴스를 인수로 `Adjust2dx::trackThirdPartySharing` 메서드를 호출하여 Adjust에 정보를 전송할 수 있습니다.
...
* `partnerName` \( **문자열** \): 세분화 옵션을 적용할 파트너의 이름.
* `key` \( **문자열** \): 옵션 키.
* `value` \( **문자열** \): 옵션 값.
...
다음 파라미터와 함께 `AdjustConfig2dx` 인스턴스에서 `setUrlStrategy` 메서드를 호출하여 데이터 레지던시 국가를 설정할 수 있습니다.

* `urlStrategy` \( **문자열** \): SDK 트래픽을 전송할 데이터 레지던시 국가 또는 엔드포인트.
