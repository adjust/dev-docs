...
---
ユーザーがAdjustリンクをクリックすると、そのアトリビューション情報が更新されます。これは、ユーザーが[ディープリンク](https://help.adjust.com/en/article/deep-links)をクリックした場合に発生します。ユーザーのアトリビューションについての情報は、`AdjustAttribution2dx`クラスに表示されます。
...
以下の値は、[`AdjustConfig2dx`インスタンスの`needsCost`プロパティ](/en/sdk/cocos2dx/configuration#enable-cost-data-sending)が`true`になっている場合にのみアクセスできます：
