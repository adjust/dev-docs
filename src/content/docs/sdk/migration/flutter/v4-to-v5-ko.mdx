...
SDK v5에서 [Adjust LinkMe](https://help.adjust.com/en/article/linkme)를 활성화하려면 `adjustConfig` 인스턴스의 `isLinkMeEnabled` 속성을 `true`로 설정해야 합니다.
...
SDK v4에서는 `adjustConfig` 인스턴스의 `playStoreKidsAppEnabled` 속성을 `true`로 설정하여 앱을 [플레이 스토어 어린이용 앱](/en/sdk/unity/features/privacy#play-store-kids-apps)으로 표시할 수 있습니다. 이 속성은 SDK 초기화 중에 읽히기 때문에 SDK가 초기화된 후에는 해당 값을 업데이트할 수 없습니다.
