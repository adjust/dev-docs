...
SDK v5에서는 이 메서드의 이름이 `enableSendingInBackground`로 변경되었습니다. 이 새 메서드는 인수를 사용하지 않습니다.
...
SDK v4에서는 부울 인수와 함께 구성 객체에 `setCoppaCompliantEnabled` 메서드를 호출하여 [COPPA 준수](/en/sdk/android/features/privacy#coppa-compliance)를 설정합니다.
...
SDK v4에서는 부울 인수와 함께 구성 객체에 `setPlayStoreKidsAppEnabled` 메서드를 호출하여 앱을 [플레이 스토어 어린이용 앱](/en/sdk/android/features/privacy#play-store-kids-apps)으로 표시할 수 있습니다.
