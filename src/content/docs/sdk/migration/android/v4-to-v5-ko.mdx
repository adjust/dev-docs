...
2. 테스트하기 전에 [앱의 SHA\-1 핑거프린트를 제공합니다](https://help.adjust.com/en/article/sdk-signature#manage-your-certificate-fingerprints).
...
SDK v4에서는 부울 인수와 함께 `AdjustConfig` 인스턴스의 `setPlayStoreKidsAppEnabled` 메서드를 호출하여 앱을 [플레이 스토어 어린이용 앱](/en/sdk/android/features/privacy#play-store-kids-apps)으로 표시할 수 있습니다.
<tabs> <tab title="Kotlin" sync="kotlin"> 
...
<tabs> <tab title="Kotlin" sync="kotlin"> <codeblock title="Google Play ADID 게터"> 
...
</CodeBlock> </Tab> <tab title="Java" sync="java"> <codeblock title="Google Play ADID 게터"> 
