...
4. (**Android only**): add the Adjust Android SDK to your project using Maven.
...
adjustConfig.setSkanUpdatedCallback([](
    std::map<std::string, std::string> skanData) {
    std::cout << "\nConversion value: " << skanData["conversionValue"];
    std::cout << "\nCoarse value: " << skanData["coarseValue"];
    std::cout << "\nLock window: " << skanData["lockWindow"];
