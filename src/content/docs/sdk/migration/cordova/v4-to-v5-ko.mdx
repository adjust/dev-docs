...
SDK v4에서는 구매 검증을 위해 `receipt`를 `AdjustEvent` 인스턴스의 `adjustEvent.setReceipt` 메서드로 전송할 수 있습니다.
...
SDK v4에서는 `receipt` 인수를 사용하여 `AdjustAppStorePurchase` 객체를 인스턴스화해야 합니다. 이 값은 구매 검증에 사용됩니다.
