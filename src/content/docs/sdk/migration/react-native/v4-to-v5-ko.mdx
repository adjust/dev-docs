...
SDK v4에서는 `true` 값으로 `adjustConfig` 인스턴스의 `setLinkMeEnabled` 메서드를 호출하여 [Adjust LinkMe](https://help.adjust.com/en/article/linkme)를 활성화할 수 있습니다.

```jsx
adjustConfig.setLinkMeEnabled(true);
```

SDK v5에서 [Adjust LinkMe](https://help.adjust.com/en/article/linkme)를 활성화하려면 `adjustConfig` 인스턴스의 `enableLinkMe` 메서드를 호출해야 합니다. 디폴트 상태는 `false`입니다.
...
SDK v4에서는 `true` 값으로 `adjustConfig` 인스턴스의 `setPlayStoreKidsAppEnabled` 메서드를 호출하여 앱을 [플레이 스토어 어린이용 앱](/en/sdk/react-native/features/privacy#play-store-kids-apps-android-only)으로 표시할 수 있습니다. SDK 초기화 중에 읽히기 때문에 SDK가 초기화된 후에는 해당 값을 업데이트할 수 없습니다.
