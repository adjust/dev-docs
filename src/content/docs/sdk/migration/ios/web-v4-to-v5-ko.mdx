---
title: "iOS 웹 브릿지 v5 마이그레이션 가이드"
description: "이 가이드에 따라 iOS 웹 브릿지를 SDK v4에서 SDK v5로 업그레이드하시기 바랍니다."
slug: "en/sdk/migration/ios/web-v4-to-v5"
sidebar-position: 2
...
### IDFA 읽기 비활성화 \{\#disable\-idfa\-reading\}

SDK v5에서는 `AdjustConfig` 인스턴스의 `disableIdfaReading` 메서드를 호출하여 Adjust SDK가 기기의 IDFA\(광고주용 ID\)를 읽는 것을 방지할 수 있습니다.
...
### 디퍼드 딥링크 열기 비활성화 \{\#disable\-deferred\-deep\-link\-opening\}
...
### SKAdNetwork 어트리뷰션 비활성화 \{\#disable\-skan\-attribution\}

SDK v5에서는 `AdjustConfig` 인스턴스의 `disableSkanAttributionHandling` 메서드를 호출하여 Adjust SDK가 어트리뷰션 정보를 Apple의 SKAdNetwork로 전송하는 것을 방지할 수 있습니다.
...
### SKAdNetwork 업데이트 콜백 \{\#skan\-update\-callback\}

SDK v5에서는 콜백 함수와 함께 `AdjustConfig` 인스턴스의 `setSkanUpdatedCallback` 메서드를 호출하여 사용자의 SKAdNetwork 전환값이 변경될 경우 Adjust SDK가 콜백 함수를 실행하도록 구성할 수 있습니다.
...
SDK v4에서는 `false` 인수와 함께 `AdjustConfig` 인스턴스의 `setAllowAdServicesInfoReading` 메서드를 호출하여 Adjust SDK가 AdServices 정보를 읽는 것을 방지할 수 있습니다.
...
SDK v4에서는 부울 인수와 함께 `AdjustConfig` 인스턴스의 `setCoppaCompliantEnabled` 메서드를 호출하여 [COPPA 준수](/en/sdk/android/features/privacy#coppa-compliance)를 설정합니다.
...
### ATT 승인 상태 요청 \{\#request\-att\-authorization\-status\}
...
명확성을 위해 SDK v5에서 이름이 `requestAppTrackingAuthorizationWithCompletionHandler`으로 변경되었습니다.
...
### 푸시 토큰 설정 \{\#set\-push\-token\}

SDK v4는 푸시 토큰과 함께 `Adjust.setDeviceToken`을 호출하여 기기 푸시 토큰을 설정하는 기능을 지원합니다.
...
SDK v4에서는 부울 인수와 함께 `AdjustConfig` 인스턴스의 `setLinkMeEnabled` 메서드를 호출하고 Adjust 웹 브릿지를 사용하여 [Adjust LinkMe](https://help.adjust.com/en/article/linkme)를 활성화할 수 있습니다.
...
### ATT 상태 확인 \{\#check\-for\-att\-status\}

SDK v4에서는 `Adjust.checkForNewAttStatus` 메서드를 호출하여 사용자의 ATT 상태 변경 사항을 확인할 수 있습니다.
