...
By default, the Adjust SDK doesn't send cost data as part of a user's attribution. You can configure the SDK to send this data by enabling cost data sending. To enable cost data sending, set the `IsCostDataInAttributionEnabled` propert on your `AdjustConfig` instance to `true`.
...
By default, the Adjust SDK pauses the sending of requests when your app is running in the background. You can configure the SDK to send requests in the background by enabling the background recording. To enable background recording, set the `IsSendingInBackgroundEnabled` property on your `AdjustConfig` instance to `true`.
