...
-  [로그 수준](/ko/sdk/unity/configuration#set-your-logging-level)을 **verbose** 로 설정합니다.

### Google Play Services 연동 테스트

Adjust SDK가 기기의 Google 광고 ID를 수신할 수 있는지 테스트하려면 [log level](/ko/sdk/unity/configuration#set-your-logging-level)을 **verbose** 로 설정하고, environment를 **Sandbox** 로 설정하시기 바랍니다. 앱을 시작하고 `session`이나 이벤트를 측정합니다. SDK가 광고 ID를 읽으면 gps_adid\(Google 플레이 서비스 광고주 ID\) 파라미터를 로깅합니다.
