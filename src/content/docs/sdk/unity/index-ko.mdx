...
* `AdServices.framework`: Apple Search Ads 트래킹에 필요
* `AdSupport.framework`: 기기의 IDFA를 읽기 위해 필요
* `AppTrackingTransparency.framework`: 사용자에게 트래킹 동의 여부를 묻기 위해 필요
...
ATT\(App Tracking Transparency\) 래퍼를 사용하는 경우, **User Tracking Description** 메시지를 입력하시기 바랍니다. 이는 사용자 화면에 트래킹 동의 다이얼로그를 표시할 때 나타납니다.
...
* `android.permission.INTERNET`: 인터넷 연결을 위해 필요합니다.
* `android.permission.ACCESS_NETWORK_STATE`: 기기가 연결된 네트워크의 유형을 읽기 위해 필요합니다.
