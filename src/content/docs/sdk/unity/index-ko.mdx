...
-  Set your [log level](/ko/sdk/unity/configuration#set-your-logging-level) to `AdjustLogLevel.Verbose`.

### Google Play Services 연동 테스트

To test that the Adjust SDK can receive a device's Google Advertising ID, set the [log level](/ko/sdk/unity/configuration#set-your-logging-level) to `AdjustLogLevel.Verbose` and the environment to `AdjustEnvironment.sandbox`. Start your app and measure a `session` or an event. The SDK logs the gps_adid \(Google Play Services Advertiser ID\) parameter if it has read the advertising ID.
