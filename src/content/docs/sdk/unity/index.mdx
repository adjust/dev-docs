...
iOS 17 introduced [Privacy manifests](https://developer.apple.com/documentation/bundleresources/privacy_manifest_files), a mechanism that informs the App Store of your app's privacy requirements. The Adjust Unity SDK doens't bundle this privacy manifest file, so you need to add the file from the Adjust iOS SDK if your app targets devices running iOS 17 or later.
...
2. Download the `PrivacyInfo.xcprivacy` file [from GitHub](https://raw.githubusercontent.com/adjust/ios_sdk/master/Adjust/PrivacyInfo.xcprivacy).
3. Copy all content from Adjust's `PrivacyInfo.xcprivacy` file to your own Privacy manifest file.
