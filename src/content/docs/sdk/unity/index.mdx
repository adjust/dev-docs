...
The Adjust SDK supports install measurement on Huawei devices using Huawei App Gallery v10.4 and later. You don't need to make any changes to start using the Huawei Referrer API.
...
2. Download the `PrivacyInfo.xcprivacy` file [from GitHub](https://raw.githubusercontent.com/adjust/ios_sdk/master/Adjust/PrivacyInfo.xcprivacy) or copy its contents.
3. Make sure the content of Adjust's `PrivacyInfo.xcprivacy` file is present in your own Privacy manifest file.
...
To enable the Signature library integration, follow the instructions in the [Signature library guide](/en/sdk/unity/integrations/signature-library).
