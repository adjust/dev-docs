...
-  Set your [log level](/ja/sdk/unity/configuration#set-your-logging-level) to `AdjustLogLevel.Verbose`.

### Google Play 開発者サービスの連携をテストする

To test that the Adjust SDK can receive a device's Google Advertising ID, set the [log level](/ja/sdk/unity/configuration#set-your-logging-level) to `AdjustLogLevel.Verbose` and the environment to `AdjustEnvironment.sandbox`. Start your app and measure a `session` or an event. The SDK logs the gps_adid \(Google Play Services Advertiser ID\) parameter if it has read the advertising ID.
