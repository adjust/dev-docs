...
* `environment` \( **AdjustEnvironment** :\) SDK를 실행하고자 하는 환경. 테스트를 위해 샌드박스 모드에서 SDK를 실행하려면 `AdjustEnvironment.sandbox`를 전달합니다. 릴리즈를 위해 프로덕션 모드에서 SDK를 실행하려면 `AdjustEnvironment.production`를 전달합니다.
