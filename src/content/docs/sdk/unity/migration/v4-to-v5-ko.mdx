...
프로젝트에서 모든 SDK v4 에셋을 제거한 뒤 [, GitHub 릴리스 페이지](https://github.com/adjust/unity_sdk/releases/tag/v5.0.0-beta1) 에서 Adjust `unitypackage` 를 다운로드하고 프로젝트로 가져옵니다.
...
`AdjustEvent.setReceiptBase64()` 메서드가 삭제되었습니다. 항상 **UnityIAP base64로 인코딩된 영수증 문자열** 을 `AdjustEvent.Receipt` 속성에 할당해야 합니다.
</Callout> 
...
SDK v4에서는 [COPPA 준수](/en/sdk/unity/features/privacy#coppa-compliance)가 `AdjustConfig` 클래스를 사용하여 설정되며 SDK 초기화 중에 읽히게 됩니다. 따라서 SDK가 초기화된 후에는 해당 값을 업데이트할 수 없습니다.
</MinorVersion> 
