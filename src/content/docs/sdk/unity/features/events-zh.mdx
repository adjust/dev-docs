...
slug: "zh/sdk/unity/features/events"
multiVersion: true
---
...
此示例展示了如何使用识别码`g3mfiw`来记录事件。该函数会将该事件的 `revenue` 属性设为 _`0.25`_ ，并将 `currency` 属性设为 _`EUR`_ 。
...
您可以为每个事件添加自定义字符串标识符来报告[事件成功和失败](/zh/sdk/unity/features/callbacks#event-callbacks)。要为事件分配回传标识符，请设置`AdjustEvent`实例的`CallbackId`属性。
