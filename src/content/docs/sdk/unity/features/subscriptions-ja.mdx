...
<tabs> <tab title="App Store" sync="appstore" icon="PlatformIos"> <codeblock title="プロパティ宣言"> 
...
</Tab> <tab title="Play Store" sync="playstore" icon="PlatformGooglePlay"> <codeblock title="プロパティ宣言"> 
...
ユーザーがサブスクリプションを購入した地域を記録することができます。これを行うには、 `AdjustPlayStoreSubscription`インスタンスの`SalesRegion`プロパティを`string`として国コードに設定します。これは、[`priceLocale`](https://developer.apple.com/documentation/storekit/skproduct/1506145-pricelocale?language=swift)オブジェクトの[`countryCode`](https://developer.apple.com/documentation/foundation/nslocale/1643060-countrycode?language=swift)としてフォーマットされる必要があります。
