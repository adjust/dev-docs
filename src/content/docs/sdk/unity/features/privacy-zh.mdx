...
* `urlStrategyDomains` \(`List<string id="sl-md0000000">`\)：接收您所发送 SDK 流量的数据驻留国家/地区或终端。
* `shouldUseSubdomains` \( `bool` \)：来源是否为子域名作为前缀。
* `isDataResidency` \( `bool` \)：域名是否用于数据驻留。

<table autoWidth="1,2"> 

|     IR: 策略     |              主要和后备域名              | 使用子域名  |  数据驻留   |
|----------------|-----------------------------------|--------|---------|
...
要让您的应用符合 COPPA \(儿童在线隐私保护法\) 规定，请将`AdjustConfig`实例的`IsCoppaComplianceEnabled`属性设置为`true`。此方法会进行如下操作：
...
将`IsCoppaComplianceEnabled`设置为`false` **不会** 重新启用第三方共享。您需要针对用户重启第三方共享。
</Callout> 
...
如果您的应用会定向到不满 13 周岁的儿童，且安装地区 **不** 位于美国，则需要将其标记为儿童应用 \(Kids App\)。这可以防止 SDK 读取设备和广告 ID \(例如：`idfa`、`gps_adid`\) 和`android_id`）。

要将应用标记为 Play Store Kids App，请将`AdjustConfig`实例的`IsPlayStoreKidsComplianceEnabled`属性设置为`true` 。
