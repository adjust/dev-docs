...
この例では、`g3mfiw`トークンでイベントを記録する方法を示します。この関数は、このイベントの`revenue`プロパティを *`0.25`* に、`currency`プロパティを *`EUR`* に設定します。
...
この例では、`g3mfiw`トークンでイベントを記録する方法を示します。以下のコールバックパラメーターが追加されました：
...
この例では、`g3mfiw`トークンでイベントを記録する方法を示します。以下のパートナーパラメーターが追加されました：
...
[イベントの成功と失敗](/en/sdk/unity/features/callbacks#event-callbacks)に関するレポートを作成するために、カスタム文字列識別子を各イベントに追加することができます。コールバック識別子をイベントに割り当てるには、 `AdjustEvent`インスタンスの`CallbackId`プロパティを設定します。
