...
이 예시는 `g3mfiw` 토큰으로 이벤트를 기록하는 방법을 보여줍니다. 이 함수는 이벤트의 `revenue` 속성을 *`0.25`* , `currency` 속성을 *`EUR`* 로 설정합니다.
...
`AdjustEvent` 키\-값 인수와 함께 `AddCallbackParameter` 메서드를 호출하여 이벤트에 콜백 파라미터를 추가합니다. 이 메서드를 여러 번 호출함으로써 여러 파라미터를 추가할 수 있습니다.
...
`AdjustEvent` 키\-값 인수와 함께 `AddPartnerParameter` 메서드를 호출하여 이벤트에 파트너 파라미터를 추가합니다. 이 메서드를 여러 번 호출함으로써 여러 파라미터를 추가할 수 있습니다.
...
각 이벤트에 맞춤 문자열 ID를 추가하여 [이벤트의 성공 및 실패](/en/sdk/unity/features/callbacks#event-callbacks) 를 보고할 수 있습니다. 이벤트에 콜백 ID를 할당하려면 `AdjustEvent` 인스턴스의 `CallbackId` 속성을 설정합니다.
