...
[アプリトラッキング承認ラッパー](app-tracking-authorization-wrapper)
-----------------------------------------------------
...
トラッキング リクエスト ダイアログのテキスト内容を特性する必要があります。これを行うには、`Info.plist`ファイルで`NSUserTrackingUsageDescription`キーにテキストを追加します。
<callout type="tip"> 
