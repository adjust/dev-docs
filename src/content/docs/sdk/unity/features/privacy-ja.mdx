...
アプリが児童オンラインプライバシー保護法（COPPA）に準拠する必要がある場合は、`AdjustConfig`インスタンスの`IsCoppaComplianceEnabled`プロパティを`true`に設定します。このメソッドは、以下を実行します。
...
`IsCoppaComplianceEnabled`を`false`に設定しても、サードパーティとの共有は再有効化 **されません** 。ユーザーの[サードパーティとのデータ共有を再度有効化する](#third-party-sharing-for-specific-users)必要があります。
</Callout> 
...
アプリが13歳未満のユーザーをターゲットとしており、インストールの地域が米国 **ではない** 場合は、子供向けアプリとして登録してください。これにより、SDKがデバイスIDと広告ID（例：`idfa`、`gps_adid`、および`android_id`）を読み取るのを防ぐことができます。

アプリをPlayストアの子供向けアプリとしてマークするには、 `AdjustConfig`インスタンスの`IsPlayStoreKidsComplianceEnabled`プロパティを`true`に設定します。
