...
versions:
   ...
   - label: v4
     value: v4
redirects:
   v4: /zh/sdk/unity/v4/features/preinstalled
---

您可以通过 Adjust SDK 来记录用户设备上的预安装应用的活动，借此发送未通过推广活动下载应用的用户信息。

通过将`AdjustConfig`实例的`IsPreinstallTrackingEnabled`属性设置为`bool`值来控制是否启用预安装监测。
...
将包含预安装信息的文件路径指定到`AdjustConfig`实例的 `PreinstallFilePath` 方法，以允许 Adjust 读取该信息。
...
2. 复制此识别码并将其指定给应用委托文件中`AdjustConfig`实例的`DefaultTracker`属性。
