...
2. 使用 `"adx_sdk"` 来源实例化一个 [`AdjustAdRevenue` 对象](/en/sdk/unity/features/ad-revenue)。
3. 添加 `AdjustAdRevenue` 对象详情。用 `ecpm` 值除以 1000 来返回广告预估收入。
4. 通过调用`Adjust.TrackAdRevenue`方法向 Adjust 发送广告收入信息。
...
1. 创建一个 AD\(X\) SDK 中 `rewardedAd.OnPaidEvent` 方法被调用时触发的新函数。该函数须以参数形式接收 eCPM \(每千次展示有效成本\)金额。
2. 使用 `AdjustConfig.AdjustAdRevenueSourceAdx` 来源实例化一个 [`AdjustAdRevenue` 对象](/en/sdk/unity/features/ad-revenue)。
3. 添加 `AdjustAdRevenue` 对象详情。用 `ecpm` 值除以 1000 来返回广告预估收入。
