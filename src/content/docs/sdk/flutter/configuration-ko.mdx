...
비용 데이터는 사용자의 [어트리뷰션 정보](/en/sdk/flutter/features/attribution)에서 액세스할 수 있습니다.
...
* `appToken` `String`: [Adjust 앱 토큰](https://help.adjust.com/en/article/app-token-and-reporting-currency#view-your-app-details)입니다.
* `environment` \(`String`\): SDK를 실행할 환경입니다. 테스트를 위해 샌드박스 모드에서 SDK를 실행하려면 `AdjustEnvironment.sandbox`를 전달합니다. 릴리즈를 위해 프로덕션 모드에서 SDK를 실행하려면 `AdjustEnvironment.production`을 전달합니다.
...
기본 설정에 따라 Adjust SDK는 사용자의 어트리뷰션에서 비용 데이터를 전송하지 않습니다. 비용 데이터 전송을 활성화하여 이 데이터를 전송하도록 SDK를 구성할 수 있습니다. 비용 데이터 전송을 활성화하려면 config 인스턴스의 `needsCost` 속성에 `bool` 값을 할당합니다.
...
기본값에 따라 Adjust SDK는 앱이 백그라운드에서 실행 중일 때 요청 전송을 일시 중지합니다. 백그라운드 기록을 활성화하여 백그라운드에서 요청을 보내도록 SDK를 구성할 수 있습니다. 백그라운드 기록을 활성화하려면 config 인스턴스의 `sendInBackground` 속성에 `bool` 값을 할당합니다.
...
config 객체에는 해당 동작을 제어하는 boolean `eventBufferingEnabled` 속성이 포함되어 있습니다. 이벤트 버퍼링을 활성화하려면 config 인스턴스의 `eventBufferingEnabled` 속성에 `bool` 값을 할당합니다.
...
기본 설정에 따라 Adjust SDK는 앱이 실행되자마자 시작됩니다. 시작할 때 사용할 수 없는 데이터를 [세션 파라미터](/en/sdk/flutter/features/session-parameters)로 전송하려면 SDK의 시작을 지연시킬 수 있습니다. 이는 고유 ID와 같은 정보를 전송할 때 유용합니다.
...
* `isOffline` \(`bool`\): 오프라인 모드를 활성화하거나 `true`, 오프라인 모드를 비활성화하려면 `false`로 설정합니다.
...
* `isEnabled` \(`bool`\): SDK를 활성화하거나 `true`, SDK를 비활성화하려면 `false`로 설정합니다.
