...
1. SDK v5는 [SDK Signature 인증](https://help.adjust.com/en/article/sdk-signature)을 기본적으로 지원합니다. 현재 SDK 서명 라이브러리를 사용하는 경우, 우선 앱에서 서명 라이브러리를 삭제해야 합니다.
...
SDK v4에서는 `adjustConfig` 인스턴스의 `linkMeEnabled` 속성을 `true` 로 설정하여 [Adjust LinkMe](https://help.adjust.com/en/article/linkme) 를 활성화할 수 있습니다.
</MinorVersion> 
...
SDK v5에서는 `adjustConfig` 인스턴스의 `isLinkMeEnabled` 속성을 `true` 로 설정하여 [Adjust LinkMe](https://help.adjust.com/en/article/linkme) 를 활성화해야 합니다.
...
SDK v4에서는 `adjustConfig` 인스턴스의 `playStoreKidsAppEnabled` 속성을 `true` 으로 설정하여 앱을 [Play Store Kids 앱](/en/sdk/unity/features/privacy#play-store-kids-apps) 으로 마킹할 수 있습니다. 이 속성은 SDK 초기화 중에 읽히기 때문에 SDK가 초기화된 후에는 해당 값을 업데이트할 수 없습니다.
</MinorVersion> 
...
</CodeBlock> <codeblock title="EU URL 전략"> 

```dart
adjustConfig.setUrlStrategy(['eu.adjust.com'], true, true);
```

</CodeBlock> <codeblock title="터키 URL 전략"> 

```dart
adjustConfig.setUrlStrategy(['tr.adjust.com'], true, true);
```

</CodeBlock> <codeblock title="US URL 전략"> 
