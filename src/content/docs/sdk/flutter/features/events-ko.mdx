...
중복 이벤트 기록을 방지하기 위해 선택적 ID를 전달할 수 있습니다. SDK는 최신 순으로 10개의 ID를 저장하고, 중복된 트랜잭션 ID가 있는 매출 이벤트를 건너뜁니다. SDK가 10개 이상의 ID를 저장하도록 설정하길 원하는 경우, `AdjustConfig` 인스턴스의 `eventDeduplicationIdsMaxSize` 에 값을 할당하여 원하는 수의 ID를 설정할 수 있습니다.

ID를 설정하려면 트랜잭션 ID를 이벤트 인스턴스의 `deduplicationId` 속성에 할당합니다.
...
</CodeBlock> \#\# \[Add a callback identifier\]\(add\-a\-callback\-identifier\) <codeblock title="속성 선언"> 
