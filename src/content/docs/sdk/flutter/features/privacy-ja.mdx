...
アプリが児童オンラインプライバシー保護法（COPPA）に準拠する必要がある場合は、configインスタンスの`coppaCompliantEnabled`プロパティに **ブーリアン** 値を割り当てます。このプロパティを`true`に設定します：
...
アプリを子供向けアプリとして表示するには、configインスタンスの`playStoreKidsAppEnabled`プロパティに **ブーリアン** 値を割り当てます。アプリを子供向けアプリとして表示するには`true`に、非子供向けアプリとして表示するには`false`に設定します。
