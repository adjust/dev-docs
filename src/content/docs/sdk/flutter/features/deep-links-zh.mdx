...
</Tab> </Tabs> </Tab> <tab title="安卓" sync="android" icon="PlatformAndroid"> 

```java
public static void appWillOpenUrl(Uri url, Context context)
```

要针对 Android 启用深度链接，请在主要活动中使用 `true` 值调用 `appWillOpenUrl`。
...
您可以向 config 实例 `deferredDeeplinkCallback` 属性添加一个函数，以此配置 Adjust SDK，使其在收到延迟深度链接时调用一个函数。该函数会以 **字符串** 参数的形式接收深度链接。
