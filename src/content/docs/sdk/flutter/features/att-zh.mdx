...
slug: "zh/sdk/flutter/features/att"
---

如果您要记录设备的广告主 ID \(即 IDFA\)，需要先展示弹窗获得用户的授权。要达到这个目的，你需要在应用中添加 Apple 的App Tracking Transparency \(ATT\) 框架。Adjust SDK 会存储用户的授权状态并在每次请求中将该信息发送至 Adjust 服务器。
...
## [应用跟踪授权包装器](app-tracking-authorization-wrapper)
...
该示例展示了，当用户与弹窗交互时，我们如何记录一个人类可读的用户授权状态描述。
