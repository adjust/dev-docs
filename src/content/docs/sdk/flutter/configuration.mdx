...
-  `isCostDataInAttributionEnabled` (`bool`): set to `true` to enable cost data sending or `false` to disable cost data sending.
...
-  `isSendingInBackgroundEnabled` (`bool`): Set to `true` to enable background sending or `false` to disable background sending.
