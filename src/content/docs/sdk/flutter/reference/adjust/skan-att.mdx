...
`statusCallback` (function): Completion handler to which the status is passed.
...
`status` (num): The user's authorization status.
...
conversionValue (num): The new conversion value.
