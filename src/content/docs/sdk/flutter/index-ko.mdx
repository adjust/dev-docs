...
## [5\. 세션 트래킹 설정](5-set-up-session-tracking)

SDK가 세션 정보를 Adjust 서버로 전달할 수 있도록 세션 트래킹을 설정해야 합니다.
...
Android 기기에 대한 세션 트래킹을 설정하려면, 앱이 포그라운드\(foreground\) 상태에서 실행 중인 경우 `Adjust.onResume()` 메서드를 호출하고, 앱이 포그라운드 상태에서 실행되지 않는 경우에는 `Adjust.onPause()` 메서드를 호출합니다. 해당 사항은 글로벌 설정이나 위젯별로 설정이 가능합니다.
