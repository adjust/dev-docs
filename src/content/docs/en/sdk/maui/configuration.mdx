...
slug: en/sdk/maui/configuration
sidebar-position: 1
---
...
There is an extra boolean optional parameter to the `AdjustConfig` constructor, that indicates whether the sdk will suppress all logging.
Pass `true` in case you want to use the `AdjustLogLevel.SUPPRESS` level.
...
If you want to use the external device ID in your business analytics, you can pass it as a [session callback parameter](). TODO add when link is live: `/en/sdk/maui/features/session-parameters`
...
Cost data is accessible in the user's [attribution information](). TODO add when link is live `/en/sdk/maui/features/attribution`
