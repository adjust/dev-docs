...
You can set an external device ID by specifying an `externalDeviceId` argument in the <Tooltip>`initSdk` method || <InitSdk /></Tooltip>. The external device ID is case-sensitive. If you have imported external device IDs, make sure the value you pass matches the imported value.
