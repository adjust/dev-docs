...
class ViewController: UIViewController {
  ...
  override func viewDidAppear(_ animated: Bool) {
    ...
    if let deferredLink = UserDefaults.standard.string(forKey: "deferredLink") {
      ...
      // TO DO: Handle the deferredLink by parsing its components
      // (example: path, query parameters) and
      // navigating to the appropriate screen.
    }
  }
    ...
    NavigationStack {
      VStack {
        ...
        if !UserDefaults.standard.bool(forKey: "hasCompletedOnboarding") {
          ...
          // Show ATT prompt after delay to ensure app is active
          DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            Adjust.requestAppTrackingAuthorizationWithCompletionHandler()
          }
          ...
          if let deferredLink = UserDefaults.standard.string(forKey: "deferredLink") {
            ...
            // TO DO: Handle deferredLink by parsing its components
            // (example: path, query parameters) and
            // navigating to the appropriate screen.
          }
        } else {
