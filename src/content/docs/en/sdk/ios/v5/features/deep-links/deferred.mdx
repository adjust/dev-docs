...
This is an example ViewController that implements deferred deep linking:
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  dispatch_after(
      ...
      dispatch_get_main_queue(), ^{
        [Adjust requestAppTrackingAuthorizationWithCompletionHandler:nil];
      });
...
This is an example ContentView that implements deferred deep linking:
    ...
    NavigationStack {
      VStack {
        ...
        if !UserDefaults.standard.bool(forKey: "hasCompletedOnboarding") {
          ...
          // Show ATT prompt after delay to ensure app is active
          DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            Adjust.requestAppTrackingAuthorizationWithCompletionHandler { _ in }
          }
...
[Facebook SDK for deferred deep linking](https://developers.facebook.com/docs/ios/deep-linking#deferred-deep-linking),
disable it to prevent interference with Adjust's deferred deep linking.
