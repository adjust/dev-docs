...
Within these methods, you'll use Adjust's `processAndResolve(_:completion:)` method, which does the following:
...
func application(
  ...
  Adjust.processAndResolve(deeplink) { resolvedLinkString in
  ...
  if ([userActivity.activityType
    ...
    if (incomingLink) {
      ...
      [Adjust processAndResolveDeeplink:deeplink
            withCompletionHandler:^(NSString *resolvedLinkString) {
...
func scene(
    ...
    // If short branded link, receive long link.
    // Otherwise, receive original link.
    let deeplink = ADJDeeplink(deeplink: urlContext.url)
    Adjust.processAndResolve(deeplink) { resolvedLinkString in
        guard let resolvedLink = URL(string: resolvedLinkString) else { return }
...
Follow the instructions in the [SwiftUI apps using AppDelegate lifecycle](#swiftui-apps-using-appdelegate-lifecycle) section. The `onOpenURL` method receives universal links and app scheme deep links when the app is installed, and the link opens the app from a background or foreground state.
