...
When users with your app installed click an Adjust link, direct deep linking opens specific content in the app.

## Setup {#setup}

iOS provides several methods for receiving direct deep links depending on your app's implementation.

Within these methods, you'll use Adjust's `processAndResolve(_:callback:)` method, which does the following:
...
Your app can then parse the processed link and navigate to the appropriate screen.
...
When implementing direct deep linking, keep these key points in mind:

1. **Link Handling Requirements**

   - Handle Adjust branded links (`brandname.go.link`) and universal links (`example.com`) equivalently. If using a domain allowlist, add your Adjust branded domain.
   - Handle universal links and app scheme deep links equivalently, as Adjust converts universal links to app scheme format when iOS doesn't support them. For example, these links should navigate to the same screen:
...
2. **App State Considerations**
   - Handle links appropriately whether your app is in "not running," background, or foreground state.
   - Consider storing deep links if they arrive during app initialization or before the user is authenticated.
   - Process stored links after your app is ready to handle navigation.

## Implementation {#implementation}

Use the implementation that matches your app:
