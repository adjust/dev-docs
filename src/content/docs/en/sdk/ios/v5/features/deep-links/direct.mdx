...
- (void)scene:(UIScene *)scene
  ...
  if (connectionOptions.userActivities.count > 0) {
    ...
    if ([userActivity.activityType
      ...
      // Create deep link object
      ADJDeeplink *deeplink =
          [[ADJDeeplink alloc] initWithDeeplink:incomingLink];
      if (!deeplink) return;
...
An optional referrer URL used to track source of the deep link/app open for better attribution/reattribution and deep linking.
E.g. For channels like SEO / Organic Search, Adjust links are not directly used. If the client's root domain has Universal Links/Android App Links implemented and triggers an app opening, For such channels, we may have to rely on signals coming from the referrer URL to attribute/reattribute users.
