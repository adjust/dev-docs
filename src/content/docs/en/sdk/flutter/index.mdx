...
Well done! You should now be able to build and run your Flutter app. <Tooltip>Enable logging || <LogLevelSig /></Tooltip> to check for any issues. Check your logs to see the `Install tracked` message.
...
-  Set your <Tooltip>log level || <LogLevelSig /></Tooltip> to **verbose**.
...
To test that the Adjust SDK can receive a device's Google Advertising ID, set the <Tooltip>log level || <LogLevelSig /></Tooltip> to **verbose** and the environment to **Sandbox**. Start your app and measure a session or an event. The SDK logs the <Abbr>`gps_adid` (Google Play Services Advertiser ID)</Abbr> parameter if it has read the advertising ID.
