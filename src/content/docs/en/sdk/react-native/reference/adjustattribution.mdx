...
slug: en/sdk/react-native/reference/adjustattribution
---
...
The following properties can be accessed by calling the <Tooltip>`getAttribution` method || <AttributionSig /></Tooltip>. Any values that aren't populated for the user are returned as a null value.

<Callout type="note">

The following values can only be accessed if the <Tooltip>`needsCost` property || <SetNeedsCostSig /></Tooltip> on your `ADJConfig` instance is `true`:
