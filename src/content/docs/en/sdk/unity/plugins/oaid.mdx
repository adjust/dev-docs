...
The <Abbr>Open Anonymous Device Identifier (OAID)</Abbr> is a unique identifier for Android devices. The <Abbr>Mobile Security Alliance (MSA)</Abbr> recommends all Chinese-manufactured devices provide an OAID. You can use the OAID to attribute and track devices where Google Play Services isn't available.

The OAID plugin provides the OAID of a device. The SDK will continue to use other identifiers as well as the OAID. The OAID is readable on all devices using the MSA SDK or <Abbr>Huawei Mobile Service (HMS)</Abbr> Core SDK on Huawei devices.
