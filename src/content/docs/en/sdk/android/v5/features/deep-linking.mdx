...
slug: en/sdk/android/v5/features/deep-links
---
...
4. For [Android App Links](https://help.adjust.com/en/article/set-up-android-app-links), add an `intent` filter to specify which URLs your app can handle. Include the `data` element with the `android:autoVerify="true"` attribute in the `intent` filter.
...
Your app will now be able to handle both URI schemes and Android App Links. When a user clicks on a link that matches either the declared URI scheme or Android App Link, your app opens automatically.

If a user clicks a link with a `deep_link` parameter containing your **scheme name**, this activity fires.
