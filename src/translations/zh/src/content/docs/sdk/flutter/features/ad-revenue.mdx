...
要发送广告展示数量记录，请为广告收入实例 `adImpressionsCount` 属性指定 **数字** 值。
...
要发送广告收入渠道，请在广告收入实例 `adRevenueNetwork` 属性上将渠道名称指定 **字符串** 值。
...
要发送广告收入单元，请在广告收入实例 `adRevenueUnit` 属性上将单元指定为 **字符串** 值。
...
要发送广告收入位置，请在广告收入实例 `adRevenuePlacement` 属性上将位置指定为 **字符串** 值。
...
您在 Adjust 控制面板中[注册回传 URL](https://help.adjust.com/zh/article/recommended-placeholders-callbacks)，SDK 监测到事件后会向您的回传 URL 发送一个 GET 请求。
