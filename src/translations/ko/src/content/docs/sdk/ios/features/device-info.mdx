...
Adjust는 각 기기에 대해 고유한 ADID\(Adjust Device ID\)를 생성합니다. `adid` 메서드를 호출하면 이 ID가 **문자열** 로 반환됩니다.
...
IDFA\(ID for Advertisers\)는 Apple 기기에 대한 기기별 고유 ID입니다. `idfa` 메서드를 호출하면 이 ID가 **문자열** 로 반환됩니다.
