   ...
   if (frontMatter.redirects) {
      for (const version in frontMatter.redirects) {
         if (frontMatter.redirects[version].startsWith("/en/")) {
            frontMatter.redirects[version] = `/${locale}/${frontMatter.redirects[version].slice(4)}`;
         }
      }
   }
...
function addNewlineAfterClosingTags(content, tags) {
   ...
   const regex = new RegExp(`(\\S)(\\s*{%\\s*/(?:${tagList})\\s*%})(?!\\n)`, "g");
   return content.replace(regex, "$1\n$2");
}

function fixHeaders(content) {
   const headerRegex = /^(\s*#+\s.*?)(\\{\\#.*?\\})/gm;
   return content.replace(headerRegex, (match, p1, p2) => p1 + p2.replace(/\\/g, ""));
}
...
function fixMdxContent(content, locale) {
   ...
   const tagRegex = new RegExp(`(<[^<>]*\\/?(?:${TAG_LIST})[^<>]*>)(\\s*)(<[^<>]*\\/?(?:${TAG_LIST})[^<>]*>)`, "g");
   content = content.replace(tagRegex, "$1\n$3");
   content = fixHeaders(content);

   // Fix escaped headers
   const headerRegex = /^(\s.*?)(\\{#.*?\\})/gm;
   content = content.replace(headerRegex, (match, p1, p2) => p1 + p2.replace(/\\/g, ""));

   // Update URLs
   content = content.replace(/https:\/\/help\.adjust\.com\/en\//g, `https://help.adjust.com/${locale}/`);
...
async function main() {
   ...
   for (const locale of locales) {
      const relevantFiles = files.filter((file) =>
         file.startsWith(`src/content/docs/${locale}/`) && (file.endsWith(".mdx") || file.endsWith(".mdoc"))
      );
